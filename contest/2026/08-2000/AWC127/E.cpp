...
int main(){
	...
	rep(i,2){
		...
		table['0'+i]=mt()%1048828087;
	}
	ll n,m;
	cin>>n>>m;
	string ss,tt;
	cin>>ss>>tt;
	RollingHash<char> s(table),t(table);
	rep(i,n)s.push_back(ss[i]);
	rep(i,m)t.push_back(tt[i]);

	rrep(i,n){
		bool f=false;
		if(s.get_hash(0,i)==t.get_hash(m-1-i,m-1))f=true;
		if(s.get_hash(n-1-i,n-1)==t.get_hash(0,i))f=true;
