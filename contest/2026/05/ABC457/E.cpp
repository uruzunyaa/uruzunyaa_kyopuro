...
vl twomin(vl a,vl b){
	for(const auto &val:b)a.push_back(val);
	sort(a.begin(),a.end());
...
int main(){
	...
	rep(i,n){
		...
		rep(j,min(2LL,(ll)left[i].size())){
			v[i].push_back(left[i][j]);
		}
	}
	SegTree<vl> seg(v,twomin,vl());
	ll q;
	cin>>q;
	while(q--){
		ll l,r;
		...
		auto itl=upper_bound(left[l].begin(),left[l].end(),r);
		if(itl==left[l].begin()){
			cout<<"No"<<endl;
			continue;
		}
		...
		auto itr=lower_bound(right[r].begin(),right[r].end(),l);
		if(itr==right[r].end()){
			cout<<"No"<<endl;
			continue;
		}
