...
vl op(vl a,vl b){
	...
	rep(i,4){
		rep(j,4){
			ll usiro=i%2;
			ll mae=j/2;
			if(usiro==1&&mae==1)continue;
			ans[(i/2)*2+j%2]=min(ans[(i/2)*2+j%2],a[i]+b[j]);
		}
	}
	if(a[4]==1){
		ans[2]=min(ans[2],b[0]);
...
void solve(){
	...
	vvl v(n,{inf,inf,inf,inf,1});
	rep(i,n)v[i][0]=a[i];

	SegTree<vl> seg(v,op,e);
	ll ans=inf;

	rep(l,n){
		if(l+k-1<n){
			ans=min(ans,seg.get(l,l+k-1)[0]);
		}
		if(l+k<n){
			ans=min(ans,seg.get(l,l+k)[0]);
		}
	}
