...
int main(){
	...
	rep(i,n)loop(j,i+1,n-1){
		cin>>ans[i][j];
		ans[j][i]=ans[i][j];
		list.push_back({ans[i][j],i,j});
	}

	sort(list.begin(),list.end());

	rep(i,list.size()){
		if(uf.issame(list[i][1],list[i][2]))continue;
		uf.unite(list[i][1],list[i][2]);
		g[list[i][1]].push_back({list[i][2],list[i][0]});
		g[list[i][2]].push_back({list[i][1],list[i][0]});
	}

	vvl now;
	rep(i,n)now.push_back(dijkstra(g,i));
