...
int main(){
	ll n,q;
	cin>>n>>q;
	SegTree<ll> siro(vl(q+1,0),sums,0);
	SegTree<ll> kuro(vl(q+1,0),sums,0);
	siro.update(0,n);
	vl lastsiro(n,0);
	vl lastkuro(n,0);
	ll ans=0;
	loop(right,1,q){
		...
		if(t==1){
			...
			r--;
			ll left=lastkuro[r];
			ans+=siro.get(left,right);
			kuro.update(left,kuro.get(left,left)-1);
			kuro.update(right,kuro.get(right,right)+1);
			lastkuro[r]=right;
		}else{
			ll c;
			cin>>c;
			c--;
			ll left=lastsiro[c];
			ans-=kuro.get(left+1,right);
			siro.update(left,siro.get(left,left)-1);
			siro.update(right,siro.get(right,right)+1);
			lastsiro[c]=right;
		}
		cout<<ans<<endl;
	}	
