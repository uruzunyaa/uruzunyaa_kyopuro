...
//メイン
int main(){
	make_fact_and_factinv(3000000);
	ll one,two,three;
	cin>>one>>two>>three;
	ll ans=0;
	loop(i,1,two){
		//iは2の仕切りの必要数
		//(two-i)個の2を(one+three+1)ヵ所に置く

		ll cnt=0;
		//1,3,1,3の置き方
		cnt+=ngrmd(one,(i+2)/2,1)*ngrmd(three,(i+1)/2,1);
		cnt%=mod;

		//3,1,3,1の置き方
		cnt+=ngrmd(three,(i+2)/2,1)*ngrmd(one,(i+1)/2,1);
		cnt%=mod;

		cnt*=ngrmd(two-i,one+three+1,0);
		cnt%=mod;
		ans+=cnt;
	}
	ans%=mod;
