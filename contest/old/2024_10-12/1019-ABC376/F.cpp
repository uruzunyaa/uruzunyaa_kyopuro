	...
	loop(i,1,q){
		if(h[i]==h[i-1]){
			...
			rep(j,n){
				...
				ll mokuhyou=t[i];
				ll hantai=j;
				if(mokuhyou<zibun)mokuhyou+=n;
				if(hantai<zibun)hantai+=n;

				if(zibun<=hantai&&hantai<=mokuhyou){
					dp[i][(mokuhyou+1)%n]=min(dp[i][(mokuhyou+1)%n],dp[i-1][j]+mokuhyou-hantai+1+mokuhyou-zibun);
				}else{
					dp[i][hantai%n]=min(dp[i][hantai%n],dp[i-1][j]+mokuhyou-zibun);
				}
				...
				mokuhyou=t[i];
				hantai=j;
				if(zibun<mokuhyou)zibun+=n;
				if(hantai<mokuhyou)hantai+=n;

				if(mokuhyou<=hantai&&hantai<=zibun){
					dp[i][(mokuhyou+n-1)%n]=min(dp[i][(mokuhyou+n-1)%n],dp[i-1][j]+hantai-mokuhyou+1+zibun-mokuhyou);
				}else{
					dp[i][hantai%n]=min(dp[i][hantai%n],dp[i-1][j]+zibun-mokuhyou);
				}
			}
		}else{
			//前回の反対の手の位置はt[i-1]で固定
			rep(j,n){
				//今度は自分の手の位置をjとして全探索
