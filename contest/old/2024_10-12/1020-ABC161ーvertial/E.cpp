...
出来る限り早く働いた時と、出来る限り遅く働いた時の被り
が
答えになる。


ただし、働く回数が多く出来る時、答え無し。
*/
...
int main(){
	...
	usiro.insert(inf);
	for(ll i=n-1;0<=i;i--){
		if(s[i]=='o'&&*usiro.begin()-i>c)usiro.insert(i);
		if(usiro.size()>k)break;
	}
	rep(i,n){
		if(mae.count(i)&&usiro.count(i))cout<<i+1<<endl;
	}
	//setdbg(mae);
	//setdbg(usiro);
	return 0;
