...
問題は大きい時で
大きい時に、N/Kが割り切れるような例は割り切った後、K未満に必ずなるので
1になる事はありえない。

よって残りは
N%K=1となるKだけ数えれば良い。
これは言い換えれば、N-1をKで割り切る事が可能なK

このKを探索する事は不可能だが、Kで割った商を探索する事を考える。
(Kは√Nより大きい事を仮定してるので商は必ず小さい)

それと、NおよびN-1も必ず答えなので、それを足して終わり。

ただし、N=2の時だけはN-1は答えにならないので注意
...
int main(){
	...
	for(ll j=2;j*j<=n-1;j++){
		ll tmp=n-1;
		if(tmp%j==0&&tmp/j>=i)ans++;
	}
	cout<<ans<<endl;
	return 0;
}
