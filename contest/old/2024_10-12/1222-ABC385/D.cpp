...
int main(){
	ll n,m,x,y;
	cin>>n>>m>>x>>y;
	unordered_map<ll,set<ll>> tx;
	unordered_map<ll,set<ll>> ty;
	rep(i,n){
		ll xx,yy;
		cin>>xx>>yy;
		tx[xx].insert(yy);
	...
	rep(i,m){
		...
		cin>>d>>c;
		if(d=='U'){
			auto it=tx[x].lower_bound(y);
			while(it!=tx[x].end()&&*it<=y+c){
				ll tmp=*it;
				it++;
				tx[x].erase(tmp);
				ty[tmp].erase(x);
		...
		if(d=='L'){
			x-=c;
			auto it=ty[y].lower_bound(x);
			while(it!=ty[y].end()&&*it<=x+c){
				ll tmp=*it;
				it++;
				ty[y].erase(tmp);
				tx[tmp].erase(y);
