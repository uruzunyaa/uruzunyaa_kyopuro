...
int main(){
	ll h,w,z,p,q;
	cin>>h>>w>>z>>p>>q;
	vvl s(h+2,vl(w+2,inf));
	rep(i,h)rep(j,w)cin>>s[i+1][j+1];

	vector<vector<bool>> d(h+2,vector<bool>(w+2,false));
	d[p][q]=true;
	priority_queue<vl> pq;
	ll ans=s[p][q];
	rep(i,4){
		pq.push({-s[p+dx[i]][q+dy[i]],p+dx[i],q+dy[i]});
		d[p+dx[i]][q+dy[i]]=true;
	}
	while(!pq.empty()){
		ll next=-pq.top()[0];
		ll x=pq.top()[1];
		ll y=pq.top()[2];
		if(next>=(ans+z-1)/z)break;
		pq.pop();
		ans+=next;
		rep(i,4){
			if(d[x+dx[i]][y+dy[i]])continue;
			pq.push({-s[x+dx[i]][y+dy[i]],x+dx[i],y+dy[i]});
