...
int main(){
	...
	ll mx=0;
	rep(i,n){
		if(s[i]=='/')mx=max(mx,1LL);
		if(t.back().first==s[i]){
			t.back().second++;
		}else{
			t.push_back({s[i],1});
		}
		if(t.at(t.size()-3).first=='1'&&t.at(t.size()-2).first=='/'&&t.at(t.size()-2).second==1&&t.at(t.size()-1).first=='2'){
			mx=max(min(t.at(t.size()-3).second,t.at(t.size()-1).second)*2+1,mx);
		}
	}
	cout<<mx<<endl;
	return 0;
