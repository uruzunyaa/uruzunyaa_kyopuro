...
SegTree<ll> segmx(vl(1<<18,0),mx,0);
SegTree<ll> segmn(vl(1<<18,0),mn,inf);

ll dfs(ll left,ll right,ll mx,ll mn){
	if(left==right){
		ll res=0;
		if(segmn.get(left,left)>mx)res+=segmn.get(left,left)-mx;
		if(segmx.get(left,left)<mn)res+=mn-segmx.get(left,left);
	}
	ll ans=0;
	return ans;
	//左区間の方が勝つ場合
	if(segmx.get(left,(right-left)/2+left)>segmx.get((right-left)/2+left+1,right)){
		//弱い区間の時刻に合わせて終える時。
		ans=dfs(left,(right-left)/2+left,min(mx,segmx.get((right-left)/2+left+1,right)),mn);
		ans=dfs(left,(right-left)/2+left,min(mx,segmx.get(left,(right-left)/2+left),mn));
