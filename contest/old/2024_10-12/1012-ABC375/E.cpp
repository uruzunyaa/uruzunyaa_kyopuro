...
int main(){
	...
	cin>>n;
	ll sum=0;
	vector<vvl> dp(n+1,vvl(501,vl(501,inf)));
	dp[0][0][0]=0;
	rep(i,n){
		ll a,b;
		cin>>a>>b;
		...
		rep(j,501)rep(k,501){
			if(a==1){
				if(j+b<501)dp[i+1][j+b][k]=min(dp[i+1][j+b][k],dp[i][j][k]);
				if(k+b<501)dp[i+1][j][k+b]=min(dp[i+1][j][k+b],dp[i][j][k]+1);
				dp[i+1][j][k]=min(dp[i+1][j][k],dp[i][j][k]+1);
			}else if(a==2){
				if(j+b<501)dp[i+1][j+b][k]=min(dp[i+1][j+b][k],dp[i][j][k]+1);
				if(k+b<501)dp[i+1][j][k+b]=min(dp[i+1][j][k+b],dp[i][j][k]);
				dp[i+1][j][k]=min(dp[i+1][j][k],dp[i][j][k]+1);
			}else{
				if(j+b<501)dp[i+1][j+b][k]=min(dp[i+1][j+b][k],dp[i][j][k]+1);
				if(k+b<501)dp[i+1][j][k+b]=min(dp[i+1][j][k+b],dp[i][j][k]+1);
