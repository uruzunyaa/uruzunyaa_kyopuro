...
int main(){
	...
	for(ll i=q-1;i>=0;i--){
		if(input[i][0]==1){
			rep(j,n)rep(k,n){
				wf[j][k]=min(wf[j][k],wf[j][a[input[i][1]]]+c[input[i][1]]+wf[b[input[i][1]]][k]);
				wf[j][k]=min(wf[j][k],wf[j][b[input[i][1]]]+c[input[i][1]]+wf[a[input[i][1]]][k]);
				wf[k][j]=wf[j][k];
			}
		}else{
			if(wf[input[i][1]][input[i][2]]==inf)ans.push_back(-1);
			else ans.push_back(wf[input[i][1]][input[i][2]]);
		}
	}
