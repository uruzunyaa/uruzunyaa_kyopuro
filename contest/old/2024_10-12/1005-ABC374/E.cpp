...
int main(){
	...
	rep(i,n){
		...
		rep(j,gd[i]*2-a+1){
			//dp[i][j*a]=p*j;
			dp[i][j+a]=min(dp[i][j+a],dp[i][j]+p);
		}
		rep(j,gd[i]*2-b+1){
			dp[i][j+b]=min(dp[i][j+b],dp[i][j]+q);
		}
		ll mn=dp[i][gd[i]*2];
		for(ll j=gd[i]*2;j>0;j--){
			dp[i][j]=min(mn,dp[i][j]);
			mn=min(mn,dp[i][j]);
		}
	}
	ll mn=0,mx=100000000000;
	while(mn!=mx){
		ll mid=(mn+mx+1)/2;
		ll sum=0;
		rep(i,n){
			sum+=dp[i][gd[i]]*((mid/(gd[i]-1)));
			ll copy=mid-(mid/(gd[i]-1));
			//if(mid==4)cout<<sum<<endl;
			sum+=dp[i][copy%gd[i]];
			//if(mid==4)cout<<sum<<endl;
			if(sum>x)break;
		}
