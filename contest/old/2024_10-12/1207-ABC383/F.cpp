...
int main(){
	ll n,x,k;
	cin>>n>>x>>k;
	unordered_map<ll,vector<pair<ll,ll>>> a;
	rep(i,n){
		ll p,u,c;
		cin>>p>>u>>c;
		a[c].push_back({p,u});
	}
	vvl dp(a.size()+1,vl(x+1,0));
	//dp[i][j]=i種類目までの色を選んだ時、コストjで得られる最大の報酬
	ll i=0;
	for(const auto & val:a){
		dp[i+1]=dp[i];
		rep(j,val.second.size()){
			ll cost=val.second[j].first;
			ll u=val.second[j].second;
			vl tmp=dp[i+1];
			rep(e,x-cost+1){
				//新たな色の場合
				dp[i+1][e+cost]=max(dp[i][e]+u+k,dp[i+1][e+cost]);

				//既に存在する色
				dp[i+1][e+cost]=max(tmp[e]+u,dp[i+1][e+cost]);
			}
		}
	...
	ll ans=0;
	rep(i,x+1)ans=max(ans,dp[a.size()][i]);
	cout<<ans<<endl;
	return 0;
}
