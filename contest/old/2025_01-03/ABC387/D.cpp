...
int main(){
	...
	ll sx,sy,gx,gy;
	loop(i,1,h)loop(j,1,w)if(s[i][j]=='S')sx=i,sy=j;
	loop(i,1,h)loop(j,1,w)if(s[i][j]=='G')gx=i,gy=j;
	vvl sk(h+2,vl(w+2,inf));
	sk[sx][sy]=0;
	...
	while(!q.empty()){
		...
		ll y=q.front().second;
		q.pop();
		ll yokotate = abs(x-sx)+abs(y-sy);
		yokotate %= 2;

		if(yokotate){
			if(sk[x+1][y]==inf&&s[x+1][y]!='#'){
				q.push({x+1,y});
				sk[x+1][y]=sk[x][y]+1;
			}
			if(sk[x-1][y]==inf&&s[x-1][y]!='#'){
