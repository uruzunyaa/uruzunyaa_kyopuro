...
int main(){
	...
	//dp[i][j][k][l][m]=使う数がjとkでl=0は境界じゃない、l=1は境界値,Mは状態。

	vector<vector<vector<vvl>>> dp(n.size(),vector<vector<vvl>>(10,vector<vvl>(10,vvl(2,0))));
	
	loop(i,1,9)rep(j,10){
		if(i==j)continue;
		if(i>n[0]-'0')break;
		if(i==n[0]-'0')dp[0][i][j][1].first=1;
		else dp[0][i][j][0].first=1;
	}
	rep(i,n.size()-1){
		loop(j,1,9){
			rep(k,10){
				dp[i+1][j][k][0].first+=dp[i][j][k][0].first+dp[i][j][k][0].second;
				dp[i+1][j][k][0].second+=dp[i][j][k][0].first+dp[i][j][k][0].second;
				if()
			}
		}
	}
