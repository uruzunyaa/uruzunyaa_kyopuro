...
int main(){
	...
	string s;
	cin>>n>>s;
	vector<vector<pair<char,ll>>> segm(n+1);
	rep(i,power(3,n)){
		//{数字、変更コスト}
		segm[0].push_back({s[i],1});
	}
	rep(i,n){
		rep(j,power(3,n-i-1)){
			vvl tmp(2);
			rep(k,3){
				tmp[segm[i][j*3+k].first-'0'].push_back(segm[i][j*3+k].second);
			}
			sort(tmp[0].begin(),tmp[0].end());
			sort(tmp[1].begin(),tmp[1].end());
			if(tmp[0].size()==0){
				segm[i+1].push_back({'1',tmp[1][0]+tmp[1][1]});
			}else if(tmp[0].size()==1){
				segm[i+1].push_back({'1',tmp[1][0]});
			}else if(tmp[0].size()==2){
				segm[i+1].push_back({'0',tmp[0][0]});
			}else{
				segm[i+1].push_back({'0',tmp[0][0]+tmp[0][1]});
			}
		}
	}
	cout<<segm[n][0].second<<endl;
	return 0;
}
