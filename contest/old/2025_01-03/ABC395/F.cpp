...
//uとdの長さの歯を、合計長さhにカットする時にあり得る、上の歯の最小値と最大値
pair<ll,ll> cuting(ll u,ll d,ll h){
	pair<ll,ll> ans;
	ll cut=u+d-h;
	ans.first=max(u-cut,0LL);
	
	if(cut<=d)ans.second=u;
	else ans.second=u-(cut-d);
	return ans;
}
...
int main(){
	...
	ll mx=inf;
	ll sum=0;
	rep(i,n)cin>>u[i]>>d[i],mx=min(mx,u[i]+d[i]),sum+=u[i]+d[i];
	...
	while(mn<mx){
		ll mid=(mn+mx+1)/2;
		dp[0]=cuting(u[0],d[0],mid);
		loop(i,1,n-1){
			dp[i]=cuting(u[i],d[i],mid);
			dp[i].first=max(dp[i-1].first-x,dp[i].first);
			dp[i].second=min(dp[i-1].second+x,dp[i].second);
