...
int main(){
	...
	SegTree<ll> seg(vl(400001,-inf),mx,-inf);
	
	ll indexzero=200000;
	ll costsum=0;

	seg.update(indexzero,0);

	rep(i,n){
		ll c;
		cin>>c;

		//ここを選ばない場合のパディング無しの最善値
		ll nowans=seg.get(indexzero,400000)+costsum;

		//cout<<nowans<<endl;
		costsum+=c;
		indexzero-=ab[i];

		seg.update(indexzero,max(seg.get(indexzero,indexzero),nowans-costsum));
	}
	
	cout<<seg.get(indexzero,400000)+costsum<<endl;
	return 0;
}
