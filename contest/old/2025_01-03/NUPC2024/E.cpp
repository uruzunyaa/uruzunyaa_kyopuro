...
int main(){
	...
	if(m==1){
		for(const auto & val:p){
			ans+=((val.second*(val.second-1)))*(n-2);
		}
		cout<<ans<<endl;
		return 0;
	}
	...
	//i=0,k=0を数える
	if(p.count(0))ans+=((p[0]*(p[0]-1)))*(n-2);

	
	//i!=0,k!=0,j=0を数える
	for(const auto & val:p){
		if(val.first==0)continue;
		ans+=(val.second*(val.second-1))*p[0];
	}
	p.erase(0);
	

	for(ll j=1;;j++){
		if(power(m,j-1)>inf/m)break;
		if(p[j]==0)continue;
		p[j]--;

		ll tmp=0;
		for(const auto & val:p){
			if(val.first%power(m,j)!=0)continue;
			//cout<<j<<" "<<val.first<<endl;
			tmp+=val.second*p[val.first/power(m,j)];
		}
