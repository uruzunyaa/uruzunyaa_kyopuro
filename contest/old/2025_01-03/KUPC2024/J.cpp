...
pair<long long, long long> findCycleSumAndSize(int N, const vector<vector<pair<int, long long>>> &graph) {
    ...
    for (int start = 0; start < N; start++) {
        ...
        while (true) {
            state[cur] = 1;
            path.push_back(cur);
            cost.push_back(graph[cur][0].second);
            int nxt = graph[cur][0].first;
            if (state[nxt] == 0) {
                cur = nxt;
            } else if (state[nxt] == 1) {
                long long s = 0;
                int idx = 0;
                while (path[idx] != nxt) idx++;
                for (int i = idx; i < (int)path.size(); i++) s += cost[i];
                return make_pair(s, (long long)path.size() - idx);
            } else {
                break;
            }
        }
...
int main(){
	...
	vector<node> g(s.size());
	for(ll i=s.size()-1;i>=0;i--){
		ll tmp=max(max(m['K'],m['U']),max(m['P'],m['C']));
		m[s[i]]=i;
		
		if(tmp>=s.size())g[i]={tmp%(ll)(s.size()),1};
		else g[i]={tmp,0};
	}

	vector<vector<node>> dp(60,vector<node>(s.size()));
	dp[0]=g;

	rep(i,59){
		rep(j,s.size()){
			dp[i+1][j]={dp[i][dp[i][j].to].to,dp[i][j].cost+dp[i][dp[i][j].to].cost};
		}
	}
	pair<ll,ll> ans={max(max(m['K'],m['U']),max(m['P'],m['C'])),0};
	rep(i,60){
		if((1LL<<i)&l)
		ans={dp[i][ans.first].to,ans.second+dp[i][ans.first].cost};
	}
