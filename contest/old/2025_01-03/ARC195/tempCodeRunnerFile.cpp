...
void solve(){
	...
	vector<pair<ll,ll>> a;
	a.push_back({aa[0],0LL});
	rep(i,n){
		if(a.back().first==aa[i])a.back().second++;
		else a.push_back({aa[i],1});
	}
	ll ans=0;
	...
	while(!a.empty()){
		ans++;
		if(a.size()>1){
			if(a[a.size()-2].first==a[a.size()-1].first){
		...
		if(a[a.size()-3].first==a[a.size()-1].first&&a[a.size()-2].second==1&&a[a.size()-3].second==1){
