...
int main(){
	...
	for(const auto & val1:aa){
		for(const auto & val2:bb){
			if(val1.first+val2.first<mx)continue;
			sum[val1.first+val2.first]+=min(val1.second,val2.second);
			if(sum[val1.first+val2.first]+cnt>=n){
				cout<<"Yes"<<endl;
				return 0;
			}
		}
