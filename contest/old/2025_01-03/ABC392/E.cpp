...
int main(){
	...
	rep(i,m){
		cin>>u[i]>>v[i];
		u[i]--,v[i]--;
		if(uf.issame(u[i],v[i]))amari.push_back(i);
		else uf.unite(u[i],v[i]),ans--;
	}
	set<ll> leader;
	rep(i,n)leader.insert(uf.root(i));
	cout<<ans<<endl;
	rep(i,ans){
		auto it=leader.begin();
		if(uf.issame(u[amari[i]],*it)){
			it++;
		}

		cout<<amari[i]+1<<" "<<u[amari[i]]+1<<" "<<(*it)+1<<endl;
		
		leader.erase(uf.root(u[amari[i]]));
		ll tmp=*it;
		leader.erase(tmp);
		uf.unite(u[amari[i]],tmp);
		leader.insert(uf.root(u[amari[i]]));
	}
	return 0;
}
