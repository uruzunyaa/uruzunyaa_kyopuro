...
int main(){
	ll n;
	cin>>n;
	vl a(n-1),b(n-1),cnt(n,0);
	bool yn=true;
	rep(i,n-1){
		cin>>a[i]>>b[i],a[i]--,b[i]--;
		cnt[a[i]]++;
		cnt[b[i]]++;
		if(cnt[a[i]]==4||cnt[b[i]]==4)yn=false;
	}
	if(yn){
		cout<<-1<<endl;
		return 0;
	...
	while(!q.empty()){
		ll tmp=q.top();
		q.pop();
		sort(mxcost[tmp].rbegin(),mxcost[tmp].rend());
		ll sum4=1;
		rep(i,min(4LL,(ll)mxcost[tmp].size()))sum4+=mxcost[tmp][i];
		mx=max(mx,sum4);

		if(g[tmp].size()!=0){
			ll sum3=1;
			rep(i,min(3LL,(ll)mxcost[tmp].size()))sum3+=mxcost[tmp][i];
			mxcost[*g[tmp].begin()].push_back(sum3);

			g[*g[tmp].begin()].erase(tmp);
			if(g[*g[tmp].begin()].size()==1)q.push(*g[tmp].begin());

			g[tmp].erase(*g[tmp].begin());
		}
