...
int main(){
	...
	rep(z,t){
		...
		//dp[j][k]=j番目の数字
		vector<map<ll,ll>> dp(d.size()+1);
		dp[0][0]=0;

		rep(j,d.size()){
			if(d.at(j)==0){
				for(const auto val:dp[j]){
					if(val.first!=0)dp[j+1][val.first-2]=val.second+1;
				}
				dp[j+1][0]=max(dp[j+1][0],max(dp[j][1],dp[j][0]));
			}else if(d.at(j)==1){
				for(const auto val:dp[j]){
					dp[j+1][val.first-1]=val.second+1;
				}
				dp[j+1][0]=max(dp[j+1][0],dp[j][0]);
			}else{
				for(const auto val:dp[j]){
					dp[j+1][val.first+d.at(j)-2]=val.second+1;
				}
			}
		}
		ll mx=0;
		for(const auto val:dp[d.size()]){
			if(val.first>=0)mx=max(mx,val.first/2+val.second);
		}
