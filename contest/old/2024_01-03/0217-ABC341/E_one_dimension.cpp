...
int main(){
	...
	string s;
	cin>>n>>q>>s;
	//c[i]=s[i]とs[i+1]が異なっているかどうかを保管
	vl c(n-1);
	rep(i,n-1){
		if(s[i]!=s[i+1])c[i]=1;
		else c[i]=0;
	}
	SegTree<ll> seg(c,put,0);
	rep(z,q){
		ll m,l,r;
		cin>>m>>l>>r,l--,r--;
		if(m==1){
			ll nl,nr;
			nl=((seg.get(l-1,l-1))+1)%2;
			nr=((seg.get(r,r))+1)%2;
			if(l!=0)seg.update(l-1,nl);
			if(r!=n-1)seg.update(r,nr);
		}else{
			if(l==r)cout<<"Yes"<<endl;
			else if(seg.get(l,r-1)==r-l)cout<<"Yes"<<endl;
			else cout<<"No"<<endl;
		}
	}
	return 0;
