...
int main(){
	...
	rep(z,t){
		...
		cin>>n;
		ll ans=inf;
		rep(one,3)rep(three,2)rep(six,3)rep(ten,3){
			if((n-1*one-three*3-six*6-ten*10)>=0&&(n-1*one-three*3-six*6-ten*10)%15==0){
				ll fifteen=(n-1*one-three*3-six*6-ten*10)/15;
				ll cost=one+three+six+ten+fifteen;
				ans=min(cost,ans);
			}
		}
		cout<<ans<<endl;
