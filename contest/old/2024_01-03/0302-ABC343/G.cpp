...
std::string comb(const std::string& a, const std::string& b) {
    int maxOverlap = 0; // 最大の重なりのサイズ
    int lenA = a.size();
    int lenB = b.size();
    ...
    // bの末尾とaの先頭の重なりも調べる（必要な場合）
    for (int i = 1; i <= std::min(lenA, lenB); ++i) {
        if (b.substr(lenB - i) == a.substr(0, i)) {
            if (i > maxOverlap) {
                maxOverlap = i;
                return b + a.substr(i); // bを先頭にする
            }
        }
    }

    return a + b.substr(maxOverlap); // aを先頭にする
}
//メイン
int main(){
	ll n;
	...
	loop(i,1,n-1){
		rep(j,b[i].size()){
			rep(k,n){
				...
				dp[b[i][j]+(1<<k)]=comb(dp[b[i][j]],s[k]);
			}
		}
	}
