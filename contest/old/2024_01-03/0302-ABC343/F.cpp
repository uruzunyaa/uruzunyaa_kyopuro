...
map<ll,ll> defo={{0,0},{-1,0}};
//メイン
map<ll,ll> po(map<ll,ll> x,map<ll,ll> y){
	map<ll,ll> ans=x;
	range(val,y){
    ...
    if (ans.size() > 2) {
        ...
        for (auto it = ans.rbegin(); it != ans.rend(); ++it) {
            ++count;
            if (count > 2) {
                // 逆イテレータを基底イテレータに変換し、削除する
                ans.erase(ans.begin(), it.base());
                break; // ループから抜け出す
            }
        }
    }
...
int main(){
	...
	SegTree<map<ll,ll>> seg(a,po,defo);
	...
	rep(z,q){
		...
		if(t==1){
			...
			ll l,r;
			cin>>l>>r,l--,r--;
			map<ll,ll> tmp=seg.get(l,r);
			cout<<tmp.begin()->second<<endl;
		}
	}
	return 0;
}
