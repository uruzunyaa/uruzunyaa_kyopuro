...
int main(){
	...
	rep(bit,1LL<<n){
		vl p;
		ll sum=0;
		rep(i,n)if(1LL<<i&bit)p.push_back(i),sum+=a[i]*b[i];
		if(sum!=h*w)continue;
		do{
			rep(m,1LL<<p.size()){
				vl aa,bb;
				rep(i,p.size()){
					if(m&1LL<<i)aa.push_back(b[p[i]]),bb.push_back(a[p[i]]);
					else aa.push_back(a[p[i]]),bb.push_back(b[p[i]]);
				}
				...
				rep(i,p.size()){
					...
					if(x+aa[i]>h||y+bb[i]>w)break;
					loop(j,x,x+aa[i]-1)loop(k,y,y+bb[i]-1){
						s[j][k]=false;
					}
				}
				bool c=true;
				rep(i,h)rep(j,w)if(s[i][j])c=false;
				if(c){
					cout<<"Yes"<<endl;
					return 0;
				}
