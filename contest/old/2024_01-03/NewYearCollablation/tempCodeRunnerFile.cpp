...
int main(){
	...
	rep(i,t){
		rep(j,n){
			...
			rep(k,SIZE){
				mii[i+1][d[j][k]]+=(mii[i][j]*INV[SIZE])%mod;
				ocha[i+1][d[j][k]]+=(ocha[i][j]*INV[SIZE])%mod;
			}
		}
		rep(j,n){
			mii[i+1][j]%=mod;
			ocha[i+1][j]%=mod;
			ll same=(mii[i+1][j]*ocha[i+1][j])%mod;
			mii[i+1][j]=(mii[i+1][j]+mod-same)%mod;
			ocha[i+1][j]=(ocha[i+1][j]+mod-same)%mod;
			ans+=same;			
		}
		ans%=mod;
	}
	...
	// cout<<endl;
	// rep(i,t){
	// 	rep(j,n)cout<<mii[i][j]<<" ";
	// 	cout<<endl;
	// }
	// cout<<endl;
	// rep(i,t){
	// 	rep(j,n)cout<<ocha[i][j]<<" ";
	// 	cout<<endl;
	// }
	return 0;
}
