...
int main(){
	...
	//imos[i][j]=数字jがi番目までに登場した回数
	vvl imos(n,vl(10,0));
	imos[0][a[0]]++;
	loop(i,1,n-1){
		rep(j,10)imos[i][j]=imos[i-1][j];
		imos[i][a[i]]++;
	...
	//dp[i][j]=i個をj番目から選んだ時に作れるかどうか。作れるなら1作れないなら0
	vvl dp(21,vl(n,0));

	ll ans=(n-1)*(n-2)/2;
	//cout<<ans<<endl;

	loop(i,3,min((ll)20,n)){
		rep(j,n-i+1){
			if(dp[i-1][j]==1||dp[i-1][j+1]==1){
				dp[i][j]=1;
				continue;
			}
			if((a[j]-a[j+i-1]+10)%2==0){
				ll mid=(a[j]+a[j+i-1])/2;
				if(imos[j+i-2][mid]-imos[j][mid]!=0){
					dp[i][j]=1;
					continue;
				}
			}
