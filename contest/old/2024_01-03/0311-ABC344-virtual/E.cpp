...
int main(){
	...
	unordered_map <ll,pair<ll,ll>> list;

	list[0]=make_pair(a[n-1],a[0]);
	list[a[n-1]]=make_pair(a[n-2],0);
	list[a[0]]=make_pair(0,a[1]);
	loop(i,1,n-2){
		list[a[i]]=make_pair(a[i-1],a[i+1]);
	}
	...
	while(q--){
		...
		if(t==1){
			ll x,y;
			cin>>x>>y;
			list[y]=make_pair(x,list[x].second);
			list[x].second=y;
			list[list[y].second].first=y;
		}else{
			ll x;
			cin>>x;
