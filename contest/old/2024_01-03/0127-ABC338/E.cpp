...
int main(){
	...
	//c[i]は距離iの点の保管されてるインデックス
	vvl c(2*n);
	vector<pair<ll,ll>> ls(2*n);
	ls[0]=make_pair(2*n-1,1);
	ls[2*n-1]=make_pair(2*n-2,0);
	loop(i,1,2*n-2){
		ls[i]=make_pair(i-1,i+1);
	}

	rep(i,n){
		cin>>a[i]>>b[i],a[i]--,b[i]--;
		ll kyori=min(abs(a[i]-b[i]),2*n-abs(a[i]-b[i]));
		if(kyori%2==0){
			cout<<"Yes"<<endl;
			return 0;
		}
	...
	loop(i,1,n+1){
		rep(j,c[i].size()){
			...
			if(ls[x].first==y){
				//xの前がy
				ls[ls[y].first].second=ls[x].second;
				ls[ls[x].second].first=ls[y].first;
			}else if(ls[x].second==y){
				//xの後がy
				ls[ls[x].first].second=ls[y].second;
				ls[ls[y].second].first=ls[x].first;
			}else{
				cout<<"Yes"<<endl;
				return 0;
			}
