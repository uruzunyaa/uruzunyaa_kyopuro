...
int main(){
	...
	//string z=cnvBase(to_string(c),10,2);
	
	string z;
	for(ll i=59;i>=0;i--){
		if(c>=power(2,i))z.push_back('1'),c-=power(2,i);
		else z.push_back('0');
	}
	ll cntz=0;
	rep(i,z.size())if(z[i]=='1')cntz++;

	// if((a+b)%2!=cntz%2||a+b<cntz){
	// 	cout<<-1<<endl;
	// 	return 0;
	// }
	string x(60,'0');
	string y(60,'0');
	while(z.size()!=60)z.insert(0,"0");
	...
	for(ll i=59;i>=0;i--){
		if(z[i]=='0'){
			if(ok.count(i))if(a>0&&b>0) x[i]='1',y[i]='1',a--,b--;
		}
	}
	
	ll ansx=0;
	rep(i,x.size()){
		ansx+=(x[x.size()-i-1]-'0')*power(2,i);
	}
	ll ansy=0;
	rep(i,y.size()){
		ansy+=(y[y.size()-i-1]-'0')*power(2,i);
	}
	if(a==0&&b==0)cout<<ansx<<" "<<ansy<<endl;
	else cout<<-1<<endl;
	return 0;
}
