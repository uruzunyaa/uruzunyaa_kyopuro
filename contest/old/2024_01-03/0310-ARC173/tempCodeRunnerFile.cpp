...
int main(){
	...
	rep(z,t){
		ll k;
		cin>>k;
		string nine=cnvBase(to_string(k),10,9);
		rep(i,nine.size()-1){
			nine[nine.size()-i-1]-=1;
			if(nine[nine.size()-i-1]<'0'){
				nine[nine.size()-i-1]+=9;
				nine[nine.size()-i-2]-=1;
			}
		}
		loop(i,1,nine.size()-1){
			if(nine[i-1]<=nine[i])nine[i]++;
		}
		cout<<nine<<endl;
	}
	return 0;
