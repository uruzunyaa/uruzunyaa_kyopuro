...
int main(){
	...
	while(!bfs.empty()){
		...
		rep(i,4){
			if(a[x+dx[i]][y+dy[i]]!='#'){
				if(b[x+dx[i]][y+dy[i]]>b[x][y]+1){
					b[x+dx[i]][y+dy[i]]=b[x][y]+1;
					if(b[x+dx[i]][y+dy[i]]<=k[x+dx[i]][y+dy[i]])b[x+dx[i]][y+dy[i]]=0;
