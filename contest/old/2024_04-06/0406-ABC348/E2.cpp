...
vvl g;//隣接リスト
vl c;
vl dp;//ノード0を頂点とした部分木の総コスト
ll mn=0,sum;

//部分木のサイズを求める、mは探索ノード、bは親ノード
ll dfs(ll m,ll b){
	ll ans=c[m];
	rep(i,g[m].size()){
		if(g[m][i]==b)continue;
...
//差分計算で、頂点ノードのコストを計算
void diff(ll now,ll m,ll b){
	mn=min(now,mn);
	rep(i,g[m].size()){
		if(g[m][i]==b)continue;
		diff(now+sum-2*dp[g[m][i]],g[m][i],m);
	}
	return;
}
//メイン
int main(){
	//入力、リサイズ
	cin>>n;
	g.resize(n),c.resize(n),dp.resize(n);
	rep(i,n-1){
		ll a,b;
		cin>>a>>b,a--,b--;
		g[a].push_back(b);
	...
	//部分木のコストを求める、全てのコスト合計を代入
	sum=dfs(0,-1);

	//頂点0とした時のコスト、0の部分を足し過ぎているので引く
	mn-=sum;
