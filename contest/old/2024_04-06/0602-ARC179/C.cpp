...
bool conpare(const ll& m1,const ll& m2){
	cout<<"? "<<m1<<" "<<m2<<endl;
	ll t;
	cin>>t;
	if(t==1)return true;
...
int main(){
	...
	cin>>n;
	vl kiroku;
	loop(i,1,n)kiroku.push_back(i);
	sort(kiroku.begin(),kiroku.end(),conpare);
	rep(i,n-1){
		cout<<"+ "<<*kiroku.begin()<<" "<<*kiroku.rbegin()<<endl;
		kiroku.erase(kiroku.begin());
		kiroku.erase(kiroku.end()-1);
		ll tmp;
		cin>>tmp;
		auto it=upper_bound(kiroku.begin(),kiroku.end(),tmp,conpare);
		kiroku.insert(it,tmp);
	}
	//vvdbg(kiroku);
	cout<<"!"<<endl;
	return 0;
