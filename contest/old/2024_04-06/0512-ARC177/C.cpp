...
int main(){
	...
	n+=2;
	c = pad(c,'#');
	priority_queue<pair<ll,pair<ll,ll>>> bfs;
	vvl k(n,vl(n,inf));
	bfs.push(make_pair(0,make_pair(1,1)));
	k[1][1]=0;
	while(!bfs.empty()){
		ll x=bfs.top().second.first;
		ll y=bfs.top().second.second;
		bfs.pop();
		rep(i,4){
			if(c[x+dx[i]][y+dy[i]]=='R'){
				if(k[x+dx[i]][y+dy[i]]>k[x][y]){
					k[x+dx[i]][y+dy[i]]=k[x][y];
					bfs.push(make_pair(-k[x][y],make_pair(x+dx[i],y+dy[i])));
				}
			}else if(c[x+dx[i]][y+dy[i]]=='B'){
				if(k[x+dx[i]][y+dy[i]]>k[x][y]+1){
					k[x+dx[i]][y+dy[i]]=k[x][y]+1;
					bfs.push(make_pair(-k[x][y]-1,make_pair(x+dx[i],y+dy[i])));
				}
			}
		}
	}
	...
	rep(i,n)rep(j,n)k[i][j]=inf;
	bfs.push(make_pair(0,make_pair(n-2,1)));
	k[n-2][1]=0;
	while(!bfs.empty()){
		ll x=bfs.top().second.first;
		ll y=bfs.top().second.second;
