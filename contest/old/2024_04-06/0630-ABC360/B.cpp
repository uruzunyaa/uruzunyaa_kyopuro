...
int main(){
	...
	cin>>s>>t;
	ll mn=s.size()/t.size();
	ll mx=(s.size()+t.size()-1)/t.size();
	mx=min(mx,(ll)s.size()-1);
	loop(i,mn,mx){
		//i文字間隔で、j文字目を初期値として始める。
		rep(j,i){
			if((t.size()-1)*i+j>=s.size())continue;
			if(t.size()*i+j<s.size())continue;
			string tmp;
			rep(k,t.size()){
				tmp.push_back(s[k*i+j]);
			}
			if(t==tmp){
				cout<<"Yes"<<endl;
				return 0;
