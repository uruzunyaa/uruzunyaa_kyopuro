...
int main(){
	...
	cin>>n>>k;
	//{(n-1)/n}^2k;
	ll one=power_mod(n-1,2*k)*power_mod(power_mod(n,2*k),mod-2);
	one %=mod;
	cout<<power_mod(n-1,2*k)<<" / "<<power_mod(n,2*k);
	ll anothor=mod+1-one;
	anothor*=n-1;
	anothor%=mod;
	anothor*=power_mod(2,mod-2);
	anothor%=mod;

	cout<<(anothor+one)%mod<<endl;
