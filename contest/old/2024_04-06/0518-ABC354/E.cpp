...
bool check(ll bit){
	...
	rep(i,list.size())rep(j,i){
		if(a[list[i]]==a[list[j]]||b[list[i]]==b[list[j]]){
			if(!check(bit+power(2,list[i])+power(2,list[j])))isdef=false;
		}
	}
	if(isdef){
		dp[bit]=false;
...
int main(){
	...
	if(check(0))cout<<"Takahashi"<<endl;
	else cout<<"Aoki"<<endl;
