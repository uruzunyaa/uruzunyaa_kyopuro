...
int main(){
	...
	vector<vvl> cost(min(h*w,k)+1,vvl(h,vl(w,-inf)));
	//cost[e][i][j]=e回目で、i,j座標にたどり着くときのコスト
	cost[0][sx][sy]=0;

	rep(e,min(h*w,k))rep(i,h)rep(j,w){
		rep(d,4){
			//枠外に行くならcontinue
			if(i+dx[d]<0||i+dx[d]>=h||j+dy[d]<0||j+dy[d]>=w)continue;
			cost[e+1][i+dx[d]][j+dy[d]]=max(cost[e+1][i+dx[d]][j+dy[d]],cost[e][i][j]+a[i+dx[d]][j+dy[d]]);
		}
	}

	ll ans=0;
	//各マスで残り回数づっと留まる場合を調査
	rep(e,min(h*w,k)+1)rep(i,h)rep(j,w){
		ans=max(ans,cost[e][i][j]+a[i][j]*(k-e));
	}
	cout<<ans<<endl;
	return 0;
}
