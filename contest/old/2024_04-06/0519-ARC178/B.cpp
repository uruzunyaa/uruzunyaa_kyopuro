...
//m桁以下の数の合計を998する関数、M＝2なら1~99の総和
ll sums(ll m){
	ll ans = power_mod(10,2*m)-power_mod(10,m)+mod;
	ans%=mod;
	ans*=power_mod(2,mod-2);
	ans%=mod;
	return ans;
}
...
int main(){
	...
	rep(w,t){
		...
		ans+=sums(a);
		ans+=mod-sums(a-1);
		if(a==b)ans+=mod-sums(a-1);

		ans%=mod;
		if(b==c){
			ll suma = power_mod(10,a)-power_mod(10,a-1)+mod;
			ll sumb = power_mod(10,b)-power_mod(10,b-1)+mod;
			suma%=mod,sumb%=mod;
			ans=suma*sumb-ans+mod;
			ans%=mod;
		}
		cout<<ans<<endl;
	}
