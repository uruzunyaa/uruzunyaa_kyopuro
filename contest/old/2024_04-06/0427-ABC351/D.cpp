...
int main(){
		...
		rep(j,w){
			if(s[i][j]=='.'&&gr[i][j]==-1){
				...
				while(!bfs.empty()){
					...
					bfs.pop();
					rep(k,4){
						if(s[x+dx[k]][y+dy[k]]=='.'&&gr[x+dx[k]][y+dy[k]]==-1){
							gr[x+dx[k]][y+dy[k]]=cnt;
							bfs.push((x+dx[k])*w+y+dy[k]);
							siz[cnt]++;
						}
						if(s[x+dx[k]][y+dy[k]]=='$'){
							c[cnt].insert((x+dx[k])*w+y+dy[k]);
						}
					}
				}
				cnt++;
	...
	ll mx=1;
	rep(i,h*w){
		//cout<<siz[i]<<" "<<(ll)(c[i].size())<<endl;
		mx=max(mx,siz[i]+(ll)(c[i].size()));
	}
	cout<<mx<<endl;
	return 0;
}
