...
int main(){
	...
	vl a(n);
	rep(i,n)cin>>a[i];
	vector<vector<pair<ll,ll>>> s(n);
	rep(i,n)s[i].push_back(make_pair(a[i],a[i]));
	MergeImosTree mit(s,po,vector<pair<ll,ll>>(0));

	ll ans=0;
	rep(i,n){
		ans+=mit.sp_get(0,i,a[i]);
	}
	cout<<ans<<endl;
