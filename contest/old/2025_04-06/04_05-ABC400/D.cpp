...
int main(){
	...
	pq.push({0,{a,b}});
	vvl ans(h+2,vl(w+2,inf));
	ans[a][b]=0;
	while(!pq.empty()){
		pair<ll,ll> tmp=pq.top().second;
		ll cost=-pq.top().first;
		pq.pop();
		if(cost>ans[tmp.first][tmp.second])continue;
		
		//通常移動
		rep(i,4){
			if(s[tmp.first+dx[i]][tmp.second+dy[i]]!='.')continue;
			if(ans[tmp.first+dx[i]][tmp.second+dy[i]]>ans[tmp.first][tmp.second]){
				ans[tmp.first+dx[i]][tmp.second+dy[i]]=ans[tmp.first][tmp.second];
				pq.push({-ans[tmp.first+dx[i]][tmp.second+dy[i]],{tmp.first+dx[i],tmp.second+dy[i]}});
			}
		}
		...
		rep(i,4){
			if(s[tmp.first+dx[i]][tmp.second+dy[i]]=='$')continue;
			if(ans[tmp.first+dx[i]][tmp.second+dy[i]]>ans[tmp.first][tmp.second]+1){
				ans[tmp.first+dx[i]][tmp.second+dy[i]]=ans[tmp.first][tmp.second]+1;
				pq.push({-ans[tmp.first+dx[i]][tmp.second+dy[i]],{tmp.first+dx[i],tmp.second+dy[i]}});
			}

			if(s[tmp.first+dx[i]*2][tmp.second+dy[i]*2]=='$')continue;
			if(ans[tmp.first+dx[i]*2][tmp.second+dy[i]*2]>ans[tmp.first][tmp.second]+1){
				ans[tmp.first+dx[i]*2][tmp.second+dy[i]*2]=ans[tmp.first][tmp.second]+1;
				pq.push({-ans[tmp.first+dx[i]*2][tmp.second+dy[i]*2],{tmp.first+dx[i]*2,tmp.second+dy[i]*2}});
			}
		}
	}
	cout<<ans[c][d]<<endl;
