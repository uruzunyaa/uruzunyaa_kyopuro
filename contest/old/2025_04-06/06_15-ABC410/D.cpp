...
int main(){
	...
	vector<vector<bool>> ans(n,vector<bool>(1LL<<10,false));
	ans[0][0]=true;
	bfs.push({0,0});
	while(!bfs.empty()){
		ll node=bfs.front().first;
		ll cost=bfs.front().second;
		bfs.pop();
		rep(i,g[node].size()){
			ll nnode=g[node][i].first;
			ll ncost=g[node][i].second^cost;
			if(ans[nnode][ncost])continue;
			ans[nnode][ncost]=true;
			bfs.push({nnode,ncost});
		}
	}
	rep(i,1LL<<10){
		if(ans[n-1][i]){
