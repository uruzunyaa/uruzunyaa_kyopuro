...
int main(){
	...
	while(!dj.empty()){
		ll tmp=-dj.top().first;
		ll now=dj.top().second;
		dj.pop();
		if(time[now]<=tmp)continue;
		time[now]=tmp;

		rep(i,g[now].size()){
			ll starttime=max(tmp,introot(g[now][i].d));
			ll nexttime=starttime+g[now][i].cost+(g[now][i].d/(starttime+1));
			dj.push({-nexttime,g[now][i].to});
		}
	}
