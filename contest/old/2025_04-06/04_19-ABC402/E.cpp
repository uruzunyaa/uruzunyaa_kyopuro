...
vector<unordered_map<ll,double>> dp;
vl s,c;
vector<double> p;
ll n;

double dfs(ll x,ll bit){
	if(dp[x].count(bit))return dp[x][bit];
	double ans=0;
	rep(i,n){
		...
		//ACする場合
		kitaiti+=(dfs(x-c[i],bit+(1LL<<i))+s[i])*p[i];

		//WAになる場合
		kitaiti+=dfs(x-c[i],bit)*(1.0-p[i]);
...
int main(){
	ll x;
	cin>>n>>x;
	dp=vector<unordered_map<ll,double>> (x+1);
	rep(i,n){
		ll ss,cc;
		double pp;
		cin>>ss>>cc>>pp;
		s.push_back(ss);
		c.push_back(cc);
		p.push_back(pp/100.0);
	}
	cout<<fixed<<setprecision(15)<<dfs(x,0)<<endl;
	return 0;
}
