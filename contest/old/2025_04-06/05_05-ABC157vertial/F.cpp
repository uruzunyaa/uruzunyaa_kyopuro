...
double check(double w,double h){
	...
	rep(i,n){
		time[i]=c[i]*sqrt((w-x[i])*(w-x[i])+(h-y[i])*(h-y[i]));
	}
...
int main(){
	...
	double fx=0,fy=0,fans=check(0,0);
	//cout<<fixed<<setprecision(15)<<fans<<endl;
	for(ll i=-1999;i<=2000;i+=2){
		for(ll j=-1999;j<=2000;j+=2){
			double tx=i/2.0;
			double ty=j/2.0;
			double tans=check(tx,ty);
			if(tans<fans){
				fans=tans;
				fx=tx;
				fy=ty;
	...
	for(ll i=-1000;i<=1000;i++){
		for(ll j=-1000;j<=1000;j++){
			double tx=fx+(i/2000.0);
			double ty=fy+(j/2000.0);
			double tans=check(tx,ty);
			if(tans<fans){
				fans=tans;
			}
