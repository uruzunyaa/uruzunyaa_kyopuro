...
int main(){
	...
	SegTree<pair<ll,ll>> segc(c,mn,{inf,inf});
	SegTree<ll> sega(a,mx,0);
	//c[i]={次に飛べる場所の最小値,自分の座標}
	ll ans=0;
	for(ll i=n-1;i>0;i--){
		//豆がなければスキップ
		ll tmp=sega.get(i,i);
		...
		//豆が既にある場所に飛ばせるならそこに飛ばして終わり
		ll b=segc.get(i,i).first;
		tmp=sega.get(b,i-1);
		if(tmp==1)continue;

		//豆が置いてある場所がないので、次に最も遠くに飛ばせる場所に豆を置く
		tmp=segc.get(b,i-1).second;
		sega.update(tmp,1);
	}
	cout<<ans<<endl;
	return 0;
