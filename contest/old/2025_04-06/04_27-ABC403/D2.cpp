...
int main(){
	...
	rep(i,n)cin>>a[i];
	vl v(1000002,inf);
	SegTree<ll> seg(v,mn,inf);
	seg.update(1000001,0);
	rep(i,n){
		ll tmp = inf;
		//a[i]-dよりも小さい
		if(a[i]-d>0){
			tmp=min(tmp,seg.get(0,a[i]-d-1));
		}
		//cout<<tmp<<endl;

		//a[i]-dより大きく、a[i]+dより小さい
		if(d!=0){
			tmp=min(tmp,seg.get(max(0LL,a[i]-d+1),min(a[i]+d-1,1000000LL)));
		}
		//cout<<tmp<<endl;

		//a[i]+dよりも大きい
		if(a[i]+d<1000000){
			tmp=min(tmp,seg.get(a[i]+d+1,1000000));
		}
		//cout<<tmp<<endl;

		tmp=min(tmp,seg.get(1000001,1000001));
		seg.allplus(1);
		seg.update(a[i],tmp);
	}
