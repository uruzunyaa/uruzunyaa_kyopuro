...
int main(){
	...
	//cout<<ans<<endl;
	rep(i,n)if(cnt[i]==1)check.push(i);
	rep(z,n){
		ll m=check.front();
		cnt[m]--;
		check.pop();
		sort(cost[m].rbegin(),cost[m].rend());
		ans=min(ans,sum-cost[m][0]-cost[m][1]);
		rep(i,g[m].size()){
			if(cnt[g[m][i].to]<=0)continue;
			else if(cnt[g[m][i].to]==2)check.push(g[m][i].to);
			cnt[g[m][i].to]--;
			cost[g[m][i].to].push_back(cost[m][0]+g[m][i].w);
		}
	}
	//vvdbg(cost);
	cout<<ans<<endl;
