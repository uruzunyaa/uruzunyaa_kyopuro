...
set<ll> divisor;
unordered_set<ll> fail;
bool dfs(ll n,string ans){
	//終わってたら答えを出力
	if(n==1){
		cout<<ans<<'*';
	...
	//残りが0を含まぬ回文なら、それを中央にして出力
	string s=to_string(n);
	bool f=true;
	rep(j,s.size()){
		if(s[j]=='0'||s[j]!=s[s.size()-j-1])f=false;
	}
	if(f){
		cout<<ans<<'*'<<n<<'*';
		rep(i,ans.size())cout<<ans[ans.size()-1-i];
	...
	if(fail.count(n))return false;

	for(const auto& val:divisor){
		string t=to_string(val);
		string revt;
		rep(j,t.size())revt.push_back(t[t.size()-1-j]);
		ll rev=stoll(revt);
		if(n%(rev*val)==0){
			if(dfs(n/rev/val,ans+"*"+t))return true;
		}
	}
	fail.insert(n);
	return false;
...
int main(){
	...
	for(ll i=2;i*i<=n;i++){
		if(n%i==0){
			...
			if(f)divisor.insert(n/i);
		}
	}
	if(!dfs(n,"1"))cout<<-1<<endl;
	return 0;
}
