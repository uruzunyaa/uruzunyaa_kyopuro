...
int main(){
	...
	h+=2,w+=2;
	vvl a(h,vl(w,0));
	vector<vector<bool>> sea(h,vector<bool>(w,true));
	vector<queue<pair<ll,ll>>> trigger(100001);
	loop(i,1,h-2)loop(j,1,w-2){
		cin>>a[i][j];
		sea[i][j]=false;
		trigger[a[i][j]].push({i,j});
	}

	loop(k,1,y){
		while(!trigger[k].empty()){
			bool f=true;
			ll x=trigger[k].front().first;
			ll y=trigger[k].front().second;
			trigger[k].pop();
			if(sea[x][y])continue;
			rep(i,4){
				if(sea[x+dx[i]][y+dy[i]])f=false;
			}
			if(f)continue;
			sea[x][y]=true;
			ans--;
			rep(i,4){
				if(!sea[x+dx[i]][y+dy[i]]&&a[x+dx[i]][y+dy[i]]<=k){
					trigger[k].push({x+dx[i],y+dy[i]});
				}
			}
		}
		cout<<ans<<endl;
