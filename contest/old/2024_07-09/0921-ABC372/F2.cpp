...
int main(){
	...
	vector<map<ll,ll>> dp(k);
	dp[g.begin()->first][g.begin()->first]=1;
	
	rep(i,k){
		for(const auto & val:dp[i]){
			ll node=val.first;
			rep(j,g[node].size()){
				auto it=g.lower_bound(g[node][j]);
				ll diff=0;
				if(it==g.end())it=g.begin(),diff+=n;
				diff+=it->first-g[node][j]+1;
				if(i+diff<k){
					dp[i+diff][it->first]+=val.second;
					dp[i+diff][it->first]%=mod;
				}else{
					ans+=val.second;
					ans%=mod;
				}
			}
			auto it=g.lower_bound(node+1);
			ll diff=0;
			if(it==g.end())it=g.begin(),diff+=n;
			diff+=it->first-node;
			if(i+diff<k){
