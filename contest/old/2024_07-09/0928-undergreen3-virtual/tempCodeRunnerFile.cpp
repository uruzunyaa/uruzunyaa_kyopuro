...
ll siki(ll x){
	...
	rep(i,q){
		if(s[i]=="max"){
			...
			ll num2=a.top();
			a.pop();
			a.push(max(num1,num2));
		}else if(s[i]=="min"){
			ll num1=a.top();
			a.pop();
			ll num2=a.top();
			a.pop();
			a.push(min(num1,num2));
		}else if(s[i]=="+"){
			//cout<<i<<endl;
			ll num1=a.top();
			a.pop();
...
int main(){
	...
	while(mn!=mx){
		ll mid=(mx+mn)/2;
		if(siki(mid)<y)mn=mid+1;
		if(siki(mid)==y)mx=mid;
		if(siki(mid)>y)mx=mid-1;
	}
	if(siki(mn)==y)cout<<mn<<endl;
	else cout<<-1<<endl;
	return 0;
}
