...
ll solve(){
	string s,x,y;
	cin>>s>>x>>y;
	ll cntzerox=0,cntonex=0,cntzeroy=0,cntoney=0;
	rep(i,x.size()){
		if(x[i]=='0')cntzerox++;
		else cntonex++;
	}
	...
	if(cntzerox==cntzeroy){
		cout<<"Yes"<<endl;
		return 0;
	}

	ll dzero=cntzerox-cntzeroy;
	ll done=cntoney-cntonex;
	...
	if((dzero*s.size())%done!=0){
		cout<<"No"<<endl;
		return 0;
	}

	ll tsize=(dzero*s.size())/done;

	ll ssize=s.size();
	
	loop(z,1,s.size()){
		if(s.size()%z!=0)continue;
		bool f=true;
		rep(i,z){
			rep(j,s.size()/z){
				if(s[i]!=s[j*z+i])f=false;
			}
		}
		if(f){
			ssize=z;
	...
	if(tsize%ssize==0)cout<<"Yes"<<endl;
	else cout<<"No"<<endl;
	return 0;
}
