...
int main(){
	...
	while(1){
		ll dx,dy;
		if(data[2]==-1)dx=data[0];
		else dx=w-data[0];
		if(data[3]==-1)dy=data[1];
		else dy=h-data[1];
		if(min(dx,dy)>=t){
			cout<<data[0]+t*data[2]<<" "<<data[1]+t*data[3]<<endl;
			return 0;
		}
		data[0]=data[0]+min(dx,dy)*data[2];
		data[1]=data[1]+min(dx,dy)*data[3];
		if(data[0]==w||data[0]==0)data[2]*=-1;
		if(data[1]==h||data[1]==0)data[3]*=-1;
		t-=min(dx,dy);
		if(pos.count(data)){
			if(pos[data]-t!=0)t%=pos[data]-t;
			else t==0;
		}
		pos[data]=t;
	}
