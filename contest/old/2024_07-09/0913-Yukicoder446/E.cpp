...
int main(){
	...
	while(mn!=mx){
		ll mid=(mn+mx+1)/2;
		ll siz=mid*2;
		ll slideleft=a[k]-a[k-1]-2;
		ll tmp=a[0]-1;
		bool f=true;
		rep(i,k){
			tmp+=siz;
			if(tmp<a[i])tmp=a[i]+1,slideleft=0;
			while(a[i+1]<tmp&&slideleft>0)tmp-=2,slideleft-=2;
			if(a[i+1]<tmp){
				f=false;
				break;
			}
			slideleft=min(slideleft,tmp-a[i]-1);
			//cout<<tmp<<" "<<slideleft<<endl;
		}
		if(tmp>a[k-1]+slideleft+2)f=false;
		if(f)mn=mid;
		else mx=mid-1;
	}
