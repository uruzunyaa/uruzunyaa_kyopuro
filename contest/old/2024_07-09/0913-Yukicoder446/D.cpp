...
int main(){
	...
	//j回のトースターを使用し終えていて
	//k回のトースターが使用中である
	vector<vvl> dp(n+1,vvl(3,vl(3,-inf)));
	dp[0][0][0]=0;
	rep(i,n){
		dp[i+1][0][0]=dp[i][0][0]+a[i][0];
		dp[i+1][0][1]=max(dp[i][0][0],dp[i][0][1])+a[i][1];
		dp[i+1][0][2]=max(max(dp[i][0][0],dp[i][0][1]),dp[i][0][2])+a[i][2];
		dp[i+1][1][0]=max(dp[i][1][0],dp[i][0][1])+a[i][0];
		dp[i+1][1][1]=max(max(dp[i][1][0],dp[i][0][2]),dp[i][1][1])+a[i][1];
		dp[i+1][2][0]=max(max(dp[i][1][1],dp[i][0][2]),dp[i][2][0])+a[i][0];
	}
	ll ans=0;
	rep(i,3)rep(j,3-i){
