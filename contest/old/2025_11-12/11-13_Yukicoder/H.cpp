...
int main(){
	cin>>n;
	priority_queue<pair<ll,vl>> list;
	loop(i,1,n){
		ll tmp=0;
		rep(j,inf){
			vl q(tmp+1,i);
		...
		vl vt;
		rep(j,tmp)vt.push_back(i);
		list.push({-tmp,vt});
	}
	while(list.top().first==0)list.pop();

	while(list.size()>1){
		vl a=list.top().second;
		list.pop();
		vl b=list.top().second;
		list.pop();
		...
		rep(i,b.size()){
			while(index!=a.size()){
				ans.insert(ans.begin()+i+index,a[index]);
				if(query(ans)){
					index++;
				}else{
					ans.erase(ans.begin()+i+index);
					break;
				}
			}
		}
		while(index<a.size())ans.push_back(a[index]),index++;
		list.push({-ans.size(),ans});
	}
	vl ans=list.top().second;
	cout<<"! ";
	vdbg(ans);
	return 0;
}
