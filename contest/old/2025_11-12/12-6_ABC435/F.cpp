...
int main(){
	...
	ll ans=0;
	rrep(i,n){
		ans=max(dp[pos[i]],ans);
		if(pos[i]!=0){
			ll tmp=seg.min_left(pos[i]-1,check);
			if(tmp!=pos[i]){
				ll mxs=seg.get(tmp,pos[i]-1);
				dp[pos[mxs]]=dp[pos[i]]+abs(pos[i]-pos[mxs]);
			}
		}

		if(pos[i]!=n-1){
			ll tmp=seg.max_right(pos[i]+1,check);
			if(tmp!=pos[i]){
				ll mxs=seg.get(pos[i]+1,tmp);
				dp[pos[mxs]]=dp[pos[i]]+abs(pos[i]-pos[mxs]);
			}
		}
