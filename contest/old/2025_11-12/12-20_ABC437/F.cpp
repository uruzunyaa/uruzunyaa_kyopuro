...
vector<pair<ll,ll>> marge(vector<pair<ll,ll>> a,vector<pair<ll,ll>> b){
	if(a.size()==0)return b;
	if(b.size()==0)return a;
	vector<pair<ll,ll>> res(4);
	rep(d,4){
		ll asum=a[d].first*dx[d]+a[d].second*dy[d];
		ll bsum=b[d].first*dx[d]+b[d].second*dy[d];
		if(asum>bsum)res[d]=a[d];
		else res[d]=b[d];
	}
	return res;
}
...
int main(){
	...
	SegTree<vector<pair<ll,ll>>> seg(v,marge,e);
	...
	while(q--){
		...
		if(t==1){
			...
			cin>>i>>x>>y;
			i--;
			vector<pair<ll,ll>> tmp={{x,y},{x,y},{x,y},{x,y}};
			seg.update(i,tmp);
		}else{
			ll l,r,x,y;
			cin>>l>>r>>x>>y;
			l--,r--;
			vector<pair<ll,ll>> tmp=seg.get(l,r);
			ll ans=0;
			rep(d,4){
				ans=max(ans,abs(tmp[d].first-x)+abs(tmp[d].second-y));
			}
			cout<<ans<<endl;
		}
	}
