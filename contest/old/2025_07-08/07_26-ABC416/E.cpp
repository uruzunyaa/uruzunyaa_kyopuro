...
int main(){
	...
	while(q--){
		...
		if(type==1){
			...
			//xを経由
			rep(i,n+1)rep(j,n+1)dist[i][j] = min(dist[i][j], dist[i][x] + dist[x][j]);
			//yを経由
			rep(i,n+1)rep(j,n+1)dist[i][j] = min(dist[i][j], dist[i][y] + dist[y][j]);
		}else if(type==2){
			ll x;
			cin>>x;
			x--;
			...
			rep(i,n+1)rep(j,n+1)dist[i][j] = min(dist[i][j], dist[i][x] + dist[x][j]);
			//nを経由
			rep(i,n+1)rep(j,n+1)dist[i][j] = min(dist[i][j], dist[i][n] + dist[n][j]);
		}else{
			ll ans=0;
			rep(i,n)rep(j,n){
