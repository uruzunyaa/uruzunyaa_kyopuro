...
int main(){
	...
	rep(i,n-1){
		rep(j,n){
			if(i%2==1){
				dp[i+1][j]=max(dp[i+1][j],dp[i][j]+a[(j+i+1)%n]);
				dp[i+1][(n+j-1)%n]=max(dp[i+1][(n+j-1)%n],dp[i][j]+a[(n+j-1)%n]);
			}else{
				ll g;
				if(a[(n+j-1)%n]>a[(j+i+1)%n])g=-1;
				else g=0;
				dp[i+1][(n+j+g)%n]=max(dp[i][j],dp[i+1][(n+j+g)%n]);
			}
		}
	}
	ll mx=-inf;

	rep(i,n)mx=max(mx,dp[n-1][i]);
