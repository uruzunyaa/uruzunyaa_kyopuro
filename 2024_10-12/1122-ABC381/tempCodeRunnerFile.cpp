...
int main(){
	...
	rep(z,q){
		...
		while(ansmx!=ansmn){
			...
			ll thlaindmx=r;
			ll thlaindmn=oneindmx+1;
			while(thlaindmx>thlaindmn){
				ll indmid=(thlaindmx+thlaindmn)/2;
				if(thla[indmid]-thla[oneindmx]<1)thlaindmn=indmid+1;
				else thlaindmx=indmid;
			}

			ll twoindmx=r;
			ll twoindmn=thlaindmx+1;
			while(twoindmx>twoindmn){
				ll indmid=(twoindmx+twoindmn)/2;
				if(two[indmid]-two[thlaindmx]<ansmid)twoindmn=indmid+1;
				else twoindmx=indmid;
			}
			
			//cout<<oneindmx<<" "<<thlaindmn<<" "<<twoindmn<<endl;
			
			if(one[oneindmn]-one[l-1]==ansmid&&thla[thlaindmn]-thla[oneindmx]==1&&two[twoindmn]-two[thlaindmx]==ansmid){
				ansmn=ansmid;
			}else{
				ansmx=ansmid-1;
			}
