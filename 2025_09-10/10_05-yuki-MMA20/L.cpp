...
int main(){
	...
	loop(i,1,n){
		...
		prevbig[i]=st.back().second;
		inv[st.back().second].push_back(i);
		st.push_back({a[i],i});
	}
	...
	SegTree<ll> seg(vl(n+1,0),sums,0);
	vl ans(q,0);
	rloop(i,n,1){
		rep(j,inv[i].size()){
			seg.update(inv[i][j],1);
		}
		rep(j,query[i].size()){
			ll right=query[i][j].first;
			ll index=query[i][j].second;
			ans[index]=seg.get(i,right);
		}
	}
