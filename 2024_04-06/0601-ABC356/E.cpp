...
int main(){
	...
	vl imos(1000001,0);
	imos[0]=0;
	rep(i,1000000)imos[i+1]=cnt[i+1]+imos[i];
	ll ans=0;
	rep(i,n){
		if(a[i]<=1000){
			ans+=w[a[i]];
			...
			loop(j,1,1000){
				if(j*a[i]-1>=1000000)break;
				ans+=(imos[min((j+1)*a[i]-1,1000000LL)]-imos[j*a[i]-1])*j;
			}
		}
	}
	ll r=2;
