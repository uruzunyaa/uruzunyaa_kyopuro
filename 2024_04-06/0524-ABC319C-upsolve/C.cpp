...
int main(){
	...
	do{
		ll tmp=1;
		vl index(9);
		rep(i,9)index[tp[i]]=i;
		rep(i,9){
			rep(j,check[tp[i]].size()){
				if(c[tp[i]]!=c[check[tp[i]][j][0]]&&c[tp[i]]!=c[check[tp[i]][j][1]]&&c[check[tp[i]][j][0]]==c[check[tp[i]][j][1]]){
					//順序は分からんけど数字的にはがっかりする場合
					//順序も本当にこの順番なら
					if(index[tp[i]]>index[check[tp[i]][j][0]]&&index[tp[i]]>index[check[tp[i]][j][1]]){
						tmp=0;
					}
				}
			}
	...
	cout<<fixed<<setprecision(13)<<(double)ans/fact(9)<<endl;
	return 0;
}
