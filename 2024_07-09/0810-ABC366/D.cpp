...
int main(){
	...
	rep(i,n)rep(j,n)rep(k,n){
		ll tmp;
		cin>>tmp;
		a[i+1][j+1][k+1]=a[i][j+1][k+1]+a[i+1][j][k+1]+a[i+1][j+1][k]-a[i][j][k+1]-a[i][j+1][k]-a[i+1][j][k]+a[i][j][k]+tmp;
	}
	...
	rep(z,q){
		...
		lx--,ly--,lz--;
		ll ans=a[rx][ry][rz]-a[lx][ry][rz]-a[rx][ly][rz]-a[rx][ry][lz]+a[rx][ly][lz]+a[lx][ry][lz]+a[lx][ly][rz]-a[lx][ly][lz];
		cout<<ans<<endl;
	}
