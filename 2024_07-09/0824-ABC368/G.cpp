...
int main(){
	...
	rep(i,n){
		cin>>bb[i];
		if(bb[i]!=1)notone.insert(i);
	}
	SegTree<ll> a(aa,pluss,plusdefo);
	//SegTree<ll> b(bb,multiple,multidefo);
	ll q;
	cin>>q;
	rep(z,q){
		ll t;
		...
		if(t==1){
			...
			//b.update(i,x);
			bb[i]=x;
			if(x==1)notone.erase(i);
			else notone.insert(i);
		}else{
			ll l,r;
			cin>>l>>r,l--,r--;
			ll ans=0;
			while(1){
				...
				if(*it>r||it==notone.end())break;
				if(*it!=l)ans+=a.get(l,*it-1);
				ans=max(ans+a.get(*it,*it),ans*bb[*it]);
				l=*it+1;
			}
			if(l<=r)ans+=a.get(l,r);
			cout<<ans<<endl;
		}
	}
	return 0;
