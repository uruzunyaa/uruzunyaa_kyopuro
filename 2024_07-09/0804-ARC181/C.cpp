...
int main(){
	ll n;
	cin>>n;
	vl p(n),q(n),trp(n),trq(n);
	rep(i,n)cin>>p[i],p[i]--,trp[p[i]]=i;
	rep(i,n)cin>>q[i],q[i]--,trq[q[i]]=i;
	unordered_map<ll,ll> tate,yoko;
	rep(i,n)tate[i]=q[i]+1,yoko[i]=q[i]+1;
	vector<string> ans(n,string(n,'0'));

	for(ll i=n-1;i*2>=n-1;i--){
		auto it=tate.begin();
		while(it!=tate.end()){
			ans[it->first][trp[i]]='1';
			it->second--;
			ll tmp = inf;
			if(it->second==0)tmp=it->first;
			it++;
			if(tmp!=inf)tate.erase(tmp);
		}
		it=yoko.begin();
		while(it!=yoko.end()){
			ans[trq[i]][it->first]='1';
			it->second--;
			ll tmp = inf;
			if(it->second==0)tmp=it->first;
			it++;
		...
		tate.erase(trq[i]),yoko.erase(trp[i]);
