...
ll mn (ll a,ll b){return min(a,b);}
...
int main(){
	...
	while(i<n){
		...
		while(i<n&&h[i]==nowh){
			wd.push_back({w[i],d[i]});
			i++;
		}
		//判定
		rep(j,wd.size()){
			if(wd[j].first==0)continue;
			ll tmp=seg.get(0,wd[j].first-1);
			if(tmp<wd[j].second){
				cout<<"Yes"<<endl;
				return 0;
			}
		...
		//セグ木追加
		rep(j,wd.size()){
			ll tmp=seg.get(wd[j].first,wd[j].first);
			tmp=min(tmp,wd[j].second);
			seg.update(wd[j].first,tmp);
		}
	}
	cout<<"No"<<endl;
	return 0;
