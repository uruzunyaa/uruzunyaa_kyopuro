...
int main(){
	...
	//dp[i][j]=i番目の桁で、jが上限フラグの時の最大値。
	vvl dp(41,vl(2,-inf));
	dp[40][1]=0;
	rrep(i,40){
		//この桁が1かどうか
		if(k&(1LL<<i)){
			dp[i][1]=dp[i+1][1]+zero[i];
			dp[i][0]=dp[i+1][1]+one[i];
		}else{
			dp[i][1]=dp[i+1][1]+one[i];
		}
		ll tmp=max(one[i],zero[i]);
		dp[i][0]=max(dp[i][0],dp[i+1][0]+tmp);
	}
