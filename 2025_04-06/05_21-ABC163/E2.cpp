...
int main(){
	...
	//dp[i][j]=i個の数字が置かれていて、内左からj個置かれてる場合の最大値
	...
	rep(i,n){
		rep(j,i+1){
			...
			dp[i+1][j+1]=max(dp[i+1][j+1],dp[i][j]+abs(j-a[i].second)*a[i].first);
			dp[i+1][j]=max(dp[i+1][j],dp[i][j]+abs(right-a[i].second)*a[i].first);
		}
	}
	ll ans=0;
	rep(i,n+1){
