...
int main(){
	ll n;
	cin>>n;
	vector<pair<pair<ll,string>,pair<ll,string>>> dp(2001,{{inf,""},{inf,""}});
	
	//dp[i]=iを達成する式で、{{最短の長さ,その式},{掛け算のみの長さ,その式}}
	dp[1]={{1,"1"},{1,"1"}};
	dp[11]={{2,"11"},{2,"11"}};
	dp[111]={{3,"111"},{3,"111"}};
	dp[1111]={{4,"1111"},{4,"1111"}};

	loop(i,1,n){
		if(i==1||i==11||i==111||i==1111)continue;
		
		//足し算
		loop(j,1,i/2){
			if(dp[j].first.first+dp[i-j].first.first+1<dp[i].first.first){
				dp[i].first.first=dp[j].first.first+dp[i-j].first.first+1;
				dp[i].first.second=dp[j].first.second+"+"+dp[i-j].first.second;
			}
		}
		dp[i].second.first=dp[i].first.first+2;
		dp[i].second.second="("+dp[i].first.second+")";

		//掛け算
		for(ll j=2;j*j<=i;j++){
			if(i%j!=0)continue;
			if(dp[j].second.first+dp[i/j].second.first+1<dp[i].second.first){
				dp[i].second.first=dp[j].second.first+dp[i/j].second.first+1;
				dp[i].second.second=dp[j].second.second+"*"+dp[i/j].second.second;
			}
		}

		if(dp[i].first.first>dp[i].second.first){
