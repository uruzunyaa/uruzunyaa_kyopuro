...
int main(){
	...
	loop(i,2,1000000){
		...
		loop(k,1,inf){
			tmpi*=i;
			if(tmpi>1000000)break;
			loop(j,i+1,1000000){
				if(tmpi*j>1000000)break;
				if(p[j])continue;
				ll tmpj=1;
				loop(k,1,inf){
					tmpj*=j;
					if(tmpi*tmpj>1000000)break;
					ans.insert(tmpi*tmpj);
				}
			}
		}
	}
	...
	while(q--){
		...
		cin>>a;
		ll tmp=sqrt(a+1);
		auto it = ans.upper_bound(tmp);
		it--;
		while(power(*it,2)>a)it--;
		cout<<power(*it,2)<<endl;
	}
	return 0;
}
