...
int main(){
	...
	loop(i,1,h)loop(j,1,w){
		if(a[i][j]=='S')node.push_back({i,j});
	}
	loop(i,1,h)loop(j,1,w){
		if(a[i][j]=='o')node.push_back({i,j});
	}
	loop(i,1,h)loop(j,1,w){
		if(a[i][j]=='G')node.push_back({i,j});
	}

	

	vvl g(node.size(),vl(node.size(),inf));
	//頂点iからのbfs
	rep(i,node.size()){
		...
		rep(j,node.size()){
			g[i][j]=dist[node[j].first][node[j].second];
		}
	}

	//cout<<"test"<<endl;

	vvl dp(1LL<<(node.size()),vl(node.size(),inf));
	dp[1][0]=0;
	ll ans=-1;
	loop(i,1,(1LL<<node.size())-1){
		if(i&(1LL<<(node.size()-1))){
			if(dp[i][node.size()-1]<=t)ans=max((int)ans,__popcount(i)-2);
		}
		rep(k,node.size()){
			rep(j,node.size()){
				if((1LL<<j)&i)continue;
				dp[i+(1LL<<j)][j]=min(dp[i+(1LL<<j)][j],dp[i][k]+g[k][j]);
			}
		}
	}
	cout<<ans<<endl;
