...
int main(){
	...
	set<pair<ll,ll>> use;
	set<pair<ll,ll>> notuse;

	notuse.insert({0,n});

	rep(i,k)use.insert({0LL,i});
	loop(i,k,n-k-1)notuse.insert({0LL,i});
	...
	rep(i,q){
		...
		use.erase({a[x],x});
		notuse.erase({a[x],x});
		if(use.size()!=k)ans-=a[x];
		a[x]=y;
		if(use.size()==k){
			
			if(use.begin()->first < y){
				notuse.insert({use.begin()->first,use.begin()->second});
				ans-=use.begin()->first;
				use.erase(use.begin());
				use.insert({a[x],x});
				ans+=y;
			...
			if(notuse.rbegin()->first < y){
				//cout<<"test"<<endl;
				use.insert({a[x],x});
				ans+=y;
			}else{
				use.insert({notuse.rbegin()->first,notuse.rbegin()->second});
				ans+=notuse.rbegin()->first;
				auto it=notuse.end();
				it--;
				notuse.erase(it);
				notuse.insert({a[x],x});
