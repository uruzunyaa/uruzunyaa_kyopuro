...
int main(){
	...
	vl ans;
	queue<ll> del;
	rep(i,n)if(g[i].size()>2)del.push(i),ans.push_back(g[i].size());

	while(!del.empty()){
		ll tmp=del.front();
		del.pop();
		for(const auto & val: g[tmp]){
			for(const auto & val2:g[val]){
				if(val2==tmp)continue;
				g[val2].erase(val);
			}
			g[val].clear();
		}
	...
	rep(i,n){
		...
		rep(j,uf.size(i)/3)ans.push_back(2);
	}
