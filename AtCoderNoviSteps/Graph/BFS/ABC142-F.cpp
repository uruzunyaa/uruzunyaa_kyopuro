...
int main(){
	...
	pair<ll,ll>none={inf,inf};
	rep(s,n){
		queue<ll> bfs;
		vector<pair<ll,ll>> dist(g.size(),none);
		dist[s]={0,inf};
		bfs.push(s);
		bool f=false;
		while(!bfs.empty()){
			...
			rep(i,g[tmp].size()){
				ll next=g[tmp][i];
				if(next==s){
					if(ans.size()==0||ans.size()>dist[tmp].first){
						ans.clear();
						ans.push_back(tmp);
						ll siz=dist[tmp].first;
						rep(i,siz){
							tmp=dist[tmp].second;
							ans.push_back(tmp);
						}
				...
				if(dist[next]!=none)continue;
				dist[next]={dist[tmp].first+1,tmp};
				bfs.push(next);
			}
			if(f)break;
		}
