...
double diff(ll x1,ll y1,ll x2,ll y2){
	ll diffx=(x1-x2)*(x1-x2);
	ll diffy=(y1-y2)*(y1-y2);
...
int main(){
	...
	do{
		rep(i,1LL<<n){
			double tmp;
			pair<ll,ll> mae;
			if(1LL&i)tmp=diff(0,0,a[p[0]],b[p[0]]),mae={c[p[0]],d[p[0]]};
			else tmp=diff(0,0,c[p[0]],d[p[0]]),mae={a[p[0]],b[p[0]]};
			loop(j,1,n-1){
				if((1LL<<j)&i)tmp+=diff(mae.first,mae.second,a[p[j]],b[p[j]]),mae={c[p[j]],d[p[j]]};
				else tmp+=diff(mae.first,mae.second,c[p[j]],d[p[j]]),mae={a[p[j]],b[p[j]]};
			}
			tmp/=s;
			ans=min(ans,tmp);
		}
	...
	rep(i,n)ans+=diff(a[i],b[i],c[i],d[i])/t;
