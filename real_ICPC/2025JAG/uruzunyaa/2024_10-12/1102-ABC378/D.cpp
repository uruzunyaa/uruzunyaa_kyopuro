...
void dfs(ll x,ll y,vector<pair<ll,ll>> & kiroku){
	if(s[x][y]=='#')return;
	pair<ll,ll>tmp={x,y};
	rep(i,kiroku.size()-1){
		if(kiroku[i]==tmp){
	...
	if(kiroku.size()==k){
		//rep(i,kiroku.size())cout<<kiroku[i].first<<','<<kiroku[i].second<<" ";
		//cout<<endl;
		ans++;
		return;
	}
	rep(i,4){
		kiroku.push_back({x+dx[i],y+dy[i]});
		dfs(x+dx[i],y+dy[i],kiroku);
		kiroku.pop_back();
	}
}
...
int main(){
	...
	s=pad(s,'#');
	vector<pair<ll,ll>> kiroku;
	loop(i,1,h)loop(j,1,w){
		
		kiroku.push_back({i,j});
		dfs(i,j,kiroku);
		kiroku.pop_back();
	}
	cout<<ans<<endl;
