...
vector<string> pet(ll n){
	if(n==0){
		vector<string> ans(1,string(1,'#'));
		return ans;
	}
	vector<string> nmo=pet(n-1);
	vector<string> ans(nmo.size()*3,string(nmo.size()*3,'.'));
	ll m=nmo.size();
	rep(i,3)rep(j,3){
		if(i==1&&j==1)continue;
		rep(k,m)rep(l,m){
			ans[i*m+k][j*m+l]=nmo[k][l];
		}
	}
	return ans;
