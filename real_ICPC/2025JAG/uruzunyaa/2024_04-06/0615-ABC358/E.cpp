...
int main(){
	...
	rep(i,26)cin>>c[i];
	vvl dp(27,vl(k+1,0));
	//dp[i][j]=i文字目までを考慮した際、j文字の選び方の通り。
	dp[0][0]=1;
	...
	rep (i,k+1)ncr[i][i] = 1;
	loop (i,1,k){
		loop (j,1,min((ll)i-1,k)) {
			//nCr= n-1Cr-1 + n-1Cr
			ncr[i][j] = ncr[i-1][j-1] + ncr[i-1][j],ncr[i][j]%=mod;
		}
	}
	//iが追加する文字の種類
	rep(i,26){
		...
		loop(j,0,k){
			//xが今追加する文字数
			loop(x,0,min(k-j,c[i])){
				dp[i+1][j+x]+=dp[i][j]*ncr[j+x][x],dp[i+1][x+j]%=mod;
			}
		}
	}
	...
	ll ans=0;
	loop(i,1,k)ans+=dp[26][i],ans%=mod;
	cout<<ans<<endl;
	//vvdbg(dp);
	return 0;
}
