...
int main(){
	...
	cin>>h>>w,h+=2,w+=2;
	vvl a(h,vl(w,-inf));
	loop(i,1,h-2)loop(j,1,w-2)cin>>a[i][j],a[i][j]*=-1;
	//vvdbg(a);
	priority_queue<pair<ll,ll>> o;
	o.push(make_pair(0,w+1));

	priority_queue<pair<ll,ll>> b;
	b.push(make_pair(0,w*(h-2)+w-2));
	...
	rep(i,inf){
		...
		if(i%2==0){
			...
			c[x][y]=1;
			rep(j,4){
				if(c[x+dx[j]][y+dy[j]]==2){
					cout<<i-1<<endl;
					return 0;
				}
				o.push(make_pair(a[x+dx[j]][y+dy[j]],(x+dx[j])*w+y+dy[j]));
			}
		}else{
			while(1){
				x=(b.top().second)/w;
			...
			c[x][y]=2;
			rep(j,4){
				if(c[x+dx[j]][y+dy[j]]==1){
					cout<<i-1<<endl;
					return 0;
				}
				b.push(make_pair(a[x+dx[j]][y+dy[j]],(x+dx[j])*w+y+dy[j]));
			}
		}
		//cout<<x<<" "<<y<<endl;
	}
