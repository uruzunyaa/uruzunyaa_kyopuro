...
ll solve(){
	...
	vector<vector<node>> gwalk(n);
	vector<vector<node>> gtaxi(n);
	rep(i,p){
		ll a,b,c;
		cin>>a>>b>>c,a--,b--;
		gwalk[a].push_back({b,a,c});
		gwalk[b].push_back({a,b,c});
	}
	rep(i,q){
		ll a,b,c;
		cin>>a>>b>>c,a--,b--;
		gtaxi[a].push_back({b,a,c});
		gtaxi[b].push_back({a,b,c});
	}

	
	//time[i]=ノードiのタクシー待ち時間を無視した時、最短時間
	vl time(n,inf);
	time[0]=0;
	...
	//待ち時間を除いたtime,ノード番号
	priority_queue<pair<ll,ll>> djwalk;
	priority_queue<pair<ll,ll>> djtaxi;
	
	djwalk.push({0,0});
	djtaxi.push({0,0});
	ll ans=inf;
	while(!djwalk.empty()){
		ll cost= -djwalk.top().first;
		ll tmp=djwalk.top().second;
		djwalk.pop();
		if(time[tmp]<cost)continue;
		rep(i,gwalk[tmp].size()){
			if(time[gwalk[tmp][i].to]>time[tmp]+gwalk[tmp][i].cost){
				time[gwalk[tmp][i].to]=time[tmp]+gwalk[tmp][i].cost;
				fastest[gwalk[tmp][i].to]=0;
				djwalk.push({-time[gwalk[tmp][i].to],gwalk[tmp][i].to});
				djtaxi.push({-time[gwalk[tmp][i].to],gwalk[tmp][i].to});
			}
		}
	}
	ans=min(ans,time[n-1]);

	
	loop(i,1,inf){
		while(!djtaxi.empty()){
			ll cost= -djtaxi.top().first;
			ll tmp=djtaxi.top().second;
			djtaxi.pop();
			if(time[tmp]<cost)continue;
			rep(j,gtaxi[tmp].size()){
				if(i>40&&i>fastest[gtaxi[tmp][j].to])continue;
				if(time[gtaxi[tmp][j].to]>time[tmp]+gtaxi[tmp][j].cost){
					time[gtaxi[tmp][j].to]=time[tmp]+gtaxi[tmp][j].cost;
					fastest[gtaxi[tmp][j].to]=min(fastest[gtaxi[tmp][j].to],i);
					djwalk.push({-time[gtaxi[tmp][j].to],gtaxi[tmp][j].to});
					djtaxi.push({-time[gtaxi[tmp][j].to],gtaxi[tmp][j].to});
				}
			}
		}
		while(!djwalk.empty()){
			...
			djwalk.pop();
			if(time[tmp]<cost)continue;
			rep(j,gwalk[tmp].size()){
				if(i>40&&i>fastest[gwalk[tmp][j].to])continue;
				if(time[gwalk[tmp][j].to]>time[tmp]+gwalk[tmp][j].cost){
					time[gwalk[tmp][j].to]=time[tmp]+gwalk[tmp][j].cost;
					fastest[gwalk[tmp][j].to]=min(fastest[gwalk[tmp][j].to],i);
					djwalk.push({-time[gwalk[tmp][j].to],gwalk[tmp][j].to});
					djtaxi.push({-time[gwalk[tmp][j].to],gwalk[tmp][j].to});
				}
			}
		}
		if(i<=40)ans=min(ans,time[n-1]+power(2,i)-1);
		else if(ans!=inf)break;
		else if(time[n-1]!=inf){
			ll two=1;
			rep(j,i)two*=2,two%=mod;
			ans=time[n-1]+two-1;
		}
		if(djwalk.empty()&&djtaxi.empty())break;
	}
