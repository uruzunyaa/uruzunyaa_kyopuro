...
ll solve(){
	...
	rep(i,m)rep(j,i){
		...
		rep(k,n)tmp[k]={a[k][i],a[k][j]};
		sort(tmp.begin(),tmp.end());
		bool f=false;
		rep(k,n-1)if(tmp[k].second>tmp[k+1].second)f=true;
		if(f){
			g[i].push_back(j);
			g[j].push_back(i);
		}
	...
	if(m>20){
		rep(i,(1LL<<(m-20))){
			...
			rep(j,m-20){
				if((1LL<<j)&i){
					rep(k,g[j+20].size()){
						if(g[j+20][k]<20){
							tmp&=(1LL<<20)-1-(1LL<<g[j+20][k]);
						}else{
							if((1LL<<(g[j+20][k]-20))&i)f=false;
						}
					}
				}
			}
		...
		rrep(i,(1LL<<20)){
			rep(j,20){
				z[i&((1LL<<20)-1-(1LL<<j))]=max(z[i&((1LL<<20)-1-(1LL<<j))],z[i]);
			}
		}
	}
	ll ans=1;
	rep(i,(1LL<<min(20LL,m))){
		bool f=true;
		//矛盾がないか調べる
		rep(j,min(20LL,m)){
			if((1LL<<j)&i){
				rep(k,g[j].size()){
					if(g[j][k]>=20)continue;
					if((1LL<<g[j][k])&i)f=false;
				}
			}
		}
		if(f)ans=max(ans,z[i]+__popcount(i));
	}
	cout<<ans<<endl;
	return 0;
}
