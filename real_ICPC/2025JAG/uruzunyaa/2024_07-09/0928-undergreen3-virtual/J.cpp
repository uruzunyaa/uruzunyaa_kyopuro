...
void dfs(vector<pair<ll,ll>> & k){
	if(k.back().first==g.first&&k.back().second==g.second){
		ans.insert(k);
	}
	rep(i,4){
		pair<ll,ll>now=k.back();
		now.first+=dx[i];
		now.second+=dy[i];
		if(now.first<=0||now.first>h||now.second<=0||now.second>w)continue;
		bool f=true;
		rep(j,k.size()-1){
			if(abs(now.first-k[j].first)+abs(now.second-k[j].second)<=1)f=false;
		}
		if(f){
			k.push_back(now);
			dfs(k);
