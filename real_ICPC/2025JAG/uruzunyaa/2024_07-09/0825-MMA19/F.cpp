	...
	while(mn!=mx){
		...
		while(!dq.empty()){
			...
			rep(i,4){
				...
				if(y+dy[i]<0||m<=y+dy[i])continue;
				ll next;
				if(a[x+dx[i]][y+dy[i]]<mid)next=1;
				else next=0;
				if(bfs[x+dx[i]][y+dy[i]]>bfs[x][y]+next){
					bfs[x+dx[i]][y+dy[i]]=bfs[x][y]+next;
					if(next==0)dq.push_front({x+dx[i],y+dy[i]});
					else dq.push_back({x+dx[i],y+dy[i]});
				}
			}
