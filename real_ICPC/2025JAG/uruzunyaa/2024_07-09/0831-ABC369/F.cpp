...
int main(){
	...
	vector<pair<ll,ll>> dp(n+1,{inf,-1});
	vl rd(n,-1);

	dp[0]={0,-1};
	ll mx=0;
	rep(i,n){
		pair<ll,ll> tmp={xy[i].second,inf};
		ll it=lower_bound(dp.begin(),dp.end(),tmp)-dp.begin();
		dp[it]={xy[i].second,i};
		mx=max(it,mx);
		rd[i]=dp[it-1].second;
	}
	...
	ll t=dp[mx].second;
	rep(i,mx){
		rep(j,h-xy[t].first)ans.push_back('D');
		rep(j,w-xy[t].second)ans.push_back('R');
		h=xy[t].first;
		w=xy[t].second;
		t=rd[t];
	}
	rep(j,h-1)ans.push_back('D');
	rep(j,w-1)ans.push_back('R');
