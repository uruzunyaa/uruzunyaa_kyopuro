...
void solve(){
	...
	cin>>n>>k;
	vl aa(n),bb(n);
	rep(i,n)cin>>aa[i],aa[i]--;
	rep(i,n)cin>>bb[i],bb[i]--;
	if(k==1){
		vl b;
		b.push_back(bb[0]);
		rep(i,n-1)if(bb[i]!=bb[i+1])b.push_back(bb[i+1]);
		rrep(i,n){
			if(b.size()==0)break;
			if(aa[i]==b.back())b.pop_back();
		}
	...
	unordered_set<ll> a;
	vvl b(n);
	rep(i,n)a.insert(aa[i]),b[bb[i]].push_back(i);
