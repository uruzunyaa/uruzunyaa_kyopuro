...
int main(){
	vl ten;
	loop(i,2,9)ten.push_back(power(10,i));
	ll q;
	cin>>q;
	while(q--){
		vvl ans(9);
		ans[0].push_back(1),ans[0].push_back(3),ans[0].push_back(7),ans[0].push_back(9);
		ll x;
		cin>>x;
		rep(i,8){
			rep(j,ans[i].size()){
				rep(k,10){
					ll tmp = (ten[i]/10)*k + ans[i][j];
					if(power_mod(tmp,tmp)%ten[i]==x%ten[i])ans[i+1].push_back(tmp);
				}
			}
		}
		sort(ans[8].begin(),ans[8].end());
		cout<<ans[8][0]<<endl;
	}
	return 0;
}
