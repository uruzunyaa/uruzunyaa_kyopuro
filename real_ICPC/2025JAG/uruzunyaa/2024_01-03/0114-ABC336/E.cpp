...
int main(){
	...
	loop(m,1,dig*9){
		...
		//dp[i][j][k][l]=i桁目の、現桁和がjで、mで割った余りがkで、lが0なら限界値でなく、存在するものの個数。
		vector<vector<vvl>> dp(dig+1,vector<vvl>(m+1,vvl(m,vl(2,0))));
		...
		rep(i,dig){
			...
			rep(j,m+1){
				...
				rep(k,m){
					//次の数字
					loop(l,0,min((ll)9,m-j)){
						//渡すDP
						dp[i+1][j+l][(k*10+l)%m][0]+=dp[i][j][k][0];
						if(dp[i][j][k][1]==1){
							if(l<n[i]-'0'){
								dp[i+1][j+l][(k*10+l)%m][0]++;
							}else if(l==n[i]-'0'){
								dp[i+1][j+l][(k*10+l)%m][1]++;
							}
						}
					}
				}
