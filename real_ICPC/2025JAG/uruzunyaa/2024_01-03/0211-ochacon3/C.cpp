...
vector<vector<pair<ll,ll>>> g(2000000);
vl s(2000000,-inf);
// 訪問済みを追跡するための配列
...
//その番号の人のスコアを返す関数
ll dfs(ll c){
	if (visited[c]) return s[c];
    visited[c] = true; // 訪問済みにする
    s[c] = 0; // s[c]を0で初期化（または適切な値で初期化）
	if(s[c]!=-inf)return s[c];
	else if(g[c].size()==0)return 0;
	else {
		s[c]=inf;
		s[c]=dfs(g[c][0].first)+g[c][0].second;
		loop(i,1,g[c].size()-1){
			if(dfs(g[c][i].first)+g[c][i].second!=s[c])mj=1;
		}
	}
	return s[c];
}
int main(){
	...
	rep(i,m){
		...
		if(v[i]<0)swap(x[i],y[i]),v[i]*=-1;
		g[x[i]].push_back(make_pair(y[i],v[i]));
	}

	rep(val,n){
		if(dfs(val)>1000000000)mj=1;
	}

	if(mj==1)cout<<"No"<<endl;
	else cout<<"Yes"<<endl;


	//g[i][j]＝人iとの強さ関係のJ番目、firstが相手、secondが自分よりいくつ強いか。
	return 0;
}
