...
ll n,h,w;
vl a(7),b(7);
bool sub(vector<vector<bool>> & r,vl & lis,ll step){
	if(step==0){
		rep(i,h)rep(j,w){
	...
	rep(i,h-a[lis.size()-step])rep(j,w-b[lis.size()-step]){
		bool f=true;
		rep(k,a[lis.size()-step]){
			rep(l,b[lis.size()-step]){
				if(i+k < h && j+l < w)if(r[i+k][j+l]){
					f=false;
					break;
				}
			}
		...
		if(f){
			rep(k,a[lis.size()-step])rep(l,b[lis.size()-step]){
				r[i+k][j+l]=true;
			}
			if(sub(r,lis,step-1))return true;
			rep(k,a[lis.size()-step])rep(l,b[lis.size()-step]){
				r[i+k][j+l]=false;
			}
		}
	}
	rep(i,h-b[lis.size()-step])rep(j,w-a[lis.size()-step]){
		bool f=true;
		rep(k,b[lis.size()-step]){
			rep(l,a[lis.size()-step]){
				if(i+k < h && j+l < w)if(r[i+k][j+l]){
					f=false;
					break;
				}
...
int main(){
	cin>>n>>h>>w;
	rep(i,n)cin>>a[i]>>b[i];
	vector<vector<bool>> r(h,vector<bool>(w,false));
	vl list(n);
	rep(i,n)list[i]=i;
	if(sub(r,list,n))cout<<"Yes"<<endl;
	else cout<<"No"<<endl;
	...
	// 		ll tmp=g[i][j];
	// 		vl lis;
	// 		rep(k,n)if(1<<k && tmp)lis.push_back(k);
	// 		ll sum=0;
	// 		rep(k,i)sum+=a[lis[k]]*b[lis[k]];
	// 		if(sum!=w*h)continue;
