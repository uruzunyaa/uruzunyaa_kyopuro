...
int main(){
	...
	//d[文字が何であるか][何個目の辺か]={自分の頂点番号,行き先の頂点番号}
	vector<vector<pair<ll,ll>>> d(26);
	vvl wf(n,vl(n,inf));
	rep(i,n)rep(j,n){
		if(c[i][j]!='-'){
			d[c[i][j]-'a'].push_back({i,j});
			wf[i][j]=1;
		}
	}
	rep(i,n)wf[i][i]=0;
	...
	while(islp){
		islp=false;
		rep(m,26){
			rep(i,d[m].size())rep(j,d[m].size()){
				if(i==j)continue;
				if(wf[d[m][i].first][d[m][j].second]>wf[d[m][i].second][d[m][j].first]+2){
					wf[d[m][i].first][d[m][j].second] = wf[d[m][i].second][d[m][j].first]+2;
					islp=true;
				}
			}
		}
	}
	rep(i,n)rep(j,n)if(wf[i][j]==inf)wf[i][j]=-1;
	vvdbg(wf);
	return 0;
}
