...
int main(){
	...
	vector<pair<ll,ll>> odd(100001,{0,0});
	rep(i,100001)odd[i].second=i;
	rep(i,n/2){
		odd[v[i*2+1]].first++;
	}
	sort(odd.rbegin(),odd.rend());

	vector<pair<ll,ll>> even(100001,{0,0});
	rep(i,100001)even[i].second=i;
	rep(i,n/2){
		even[v[i*2]].first++;
	}
	...
	ll mx=0;
	if(even[0].second!=odd[0].second)mx=odd[0].first+even[0].first;
	else {
		mx=max(odd[1].first+even[0].first,odd[0].first+even[1].first);
	}
	cout<<n-mx<<endl;
