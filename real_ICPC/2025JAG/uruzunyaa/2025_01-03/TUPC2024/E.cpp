...
void solve(){
	...
	if(cntzero%2==0){
		...
		rep(i,n){
			if(s[i]=='0')cnt2++;
			if(cnt2==cntzero/2&&s[i]=='1'){
				cout<<(cntzero/2)+cntone-1<<endl;
				return;
			}
		}
	}
	...
	ll mid = cntzero-left-right;
	if(right<left)swap(right,left);

	if(mid>left+right){
		//真ん中が多い場合、中の0が消えないので、1は2つ残る
		cout<<right+left+cntone-2<<endl;
		return;
	}else if(left+mid<right){
		//右が多いので右に0が残る。1は1個
		cout<<left+mid+cntone-1<<endl;
		return;
	}else if(s[0]=='1'&&s[n-1]=='1'&&cntzero%2==1){
		cout<<(cntzero/2)+cntone-2<<endl;
		return;
	}else{
		//他の場合は理論値消える
		cout<<(cntzero/2)+cntone-1<<endl;
