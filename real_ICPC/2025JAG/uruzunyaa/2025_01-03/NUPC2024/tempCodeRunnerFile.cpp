...
int main(){
	...
	while(!leaf.empty()){
		...
		ll next=*g[tmp].begin();
		if(a.first==tmp)a.first=next,a.second++;
		if(b.first==tmp)b.first=next,b.second++;

		g[next].erase(tmp);
		g[tmp].erase(next);
		if(g[next].size()==1)leaf.push(next);
		siz--;
	}
	if(siz<=3){
		cout<<"Alice"<<endl;
	...
	pair<ll,ll> migi,hidari;
	migi={*g[a.first].begin(),1};
	leaf.push(*g[a.first].begin());
	g[*g[a.first].begin()].erase(a.first);
	g[a.first].erase(*g[a.first].begin());

	hidari={*g[a.first].begin(),1};
	leaf.push(*g[a.first].begin());
	g[*g[a.first].begin()].erase(a.first);
	g[a.first].erase(*g[a.first].begin());
	...
	while(!leaf.empty()){
		...
		if(tmp==b.first)continue;
		
		ll next=*g[tmp].begin();
		if(migi.first==tmp)migi.first=next,migi.second++;
		if(hidari.first==tmp)hidari.first=next,hidari.second++;

		g[next].erase(tmp);
		g[tmp].erase(next);
		if(g[next].size()==1)leaf.push(next);
	}
	a.second+=min(migi.second,hidari.second);

	cout<<migi.first<<" "<<migi.second<<endl;
	cout<<hidari.first<<" "<<hidari.second<<endl;

	if(a.second-1<=b.second)cout<<"Alice"<<endl;
	else cout<<"Bob"<<endl;
	return 0;
}
