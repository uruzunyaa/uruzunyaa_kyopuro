...
vector<vector<pair<ll,ll>>> mae;
void dfs(vl & m){
	rep(i,g[m.back()].size()){
		if(m.size()!=1){
			if(g[m.back()][i]==m[m.size()-2])continue;
		}
		m.push_back(g[m.back()][i]);
		dfs(m);
		m.pop_back();
	}
	if(m.size()==1)return;
	pq.push({(ll)m.size(),{m.front(),m.back()}});
	mae[m.front()][m.back()]={m[1],m[m.size()-2]};
}
...
int main(){
	...
	g=vvl(n);
	vvl a(n,vl(n,0));
	mae=vector<vector<pair<ll,ll>>>(n,vector<pair<ll,ll>>(n));
	rep(i,n-1){
		ll u,v;
		cin>>u>>v;
		u--,v--;
	...
	vector<pair<ll,pair<ll,ll>>> check;
	while(!pq.empty()){
		check.push_back(pq.top());
		ll i=pq.top().second.first;
		ll j=pq.top().second.second;
		pq.pop();
		if(!a[i][j])continue;
		a[mae[i][j].first][mae[i][j].second]=1;
	}

	UnionFind uf(n);
	rep(i,n)rep(j,n)if(a[i][j])uf.unite(i,j);

	//vvdbg(a);

	while(check.size()){
		ll siz=check.back().first;
		ll i=check.back().second.first;
		ll j=check.back().second.second;
		check.pop_back();
		if(a[i][j])continue;
		if(uf.issame(i,j)){
			if(siz==2||(a[mae[i][j].first][mae[i][j].second]==1)){
				a[i][j]=1;
			}
		}
	}
