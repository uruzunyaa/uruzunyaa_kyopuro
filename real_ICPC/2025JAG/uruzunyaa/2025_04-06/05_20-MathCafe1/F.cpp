...
int main(){
	...
	vector<pair<double,pair<ll,ll>>> t;
	loop(i,1,n-1){
		t.push_back({((double)p[i].second)/p[i].first,p[i]});
	}
	sort(t.begin(),t.end());
	t.push_back({inf,p[0]});
	t.push_back(t[0]);
	...
	rep(i,n){
		// 準備の計算
		ll vx1 = t[i+2].second.first - t[i+1].second.first;
		ll vy1 = t[i+2].second.second - t[i+1].second.second;
		ll vx2 = t[i].second.first - t[i+1].second.first;
		ll vy2 = t[i].second.second - t[i+1].second.second;
		// 判断用の値
		ll ans = vx1 * vy2 - vy1 * vx2;
		if(ans<=0){
			cout<<"No"<<endl;
			return 0;
		}
