...
int main(){
	...
	//dp[i][j][k]=i番目までの要素を見た時、加算操作がjの状態、掛け算操作がkの状態の時の答えの最大
	...
	rep(i,n){
		rep(j,3){
			rep(k,3){
				...
				//両方使用への遷移
				if(j!=2&&k!=2)dp[i+1][1][1]=max(dp[i+1][1][1],dp[i][j][k]+(a[i]+x)*y);

				//jのみ使用への遷移
				if(j!=2)dp[i+1][1][nk]=max(dp[i+1][1][nk],dp[i][j][k]+a[i]+x);

				//kのみ使用への遷移
				if(k!=2)dp[i+1][nj][1]=max(dp[i+1][nj][1],dp[i][j][k]+a[i]*y);
			}
		}
	}
	ll ans=-inf;
