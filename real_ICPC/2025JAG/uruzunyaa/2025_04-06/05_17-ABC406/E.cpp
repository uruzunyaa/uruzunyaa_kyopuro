...
void solve(){
	...
	//dp[i][j][b]
	//= 下からi番目の桁までにj個の1を使うものの{総合計,総数}
	//bは上界になっている状態かどうか。
	vector<vector<vector<pair<ll,ll>>>> dp(61,vector<vector<pair<ll,ll>>>(k+1,vector<pair<ll,ll>>(2,{0,0})));
	dp[60][0][1]={1,0};
		...
		rep(j,k+1){
			rep(b,2){
				...
				if(b==1&&!((1LL<<i)&n)){
					...
					dp[i][j][0].first+=dp[i+1][j][b].first;
					dp[i][j][0].second+=dp[i+1][j][b].second;
					dp[i][j][0].second%=mod;
				}
				//次の桁に1を使う場合
				if(j==k)continue;
				if(b==1){
					dp[i][j+1][1].first+=dp[i+1][j][1].first;
					dp[i][j+1][1].second+=dp[i+1][j][1].second+dp[i+1][j][1].first*(1LL<<i);
					dp[i][j+1][1].second%=mod;
				}else{
					dp[i][j+1][0].first+=dp[i+1][j][0].first;
					dp[i][j+1][0].second+=dp[i+1][j][0].second+dp[i+1][j][0].first*(1LL<<i);
					dp[i][j+1][0].second%=mod;
				}
			}
		}
	}
	ll ans=dp[0][k][0].second+dp[0][k][1].second;
	ans%=mod;
	cout<<ans<<endl;

	//cout<<dp[0][k][0].first+dp[0][k][1].first<<endl;
}
