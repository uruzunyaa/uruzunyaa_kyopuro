...
int main(){
	...
	while(q--){
		...
		if(t==1){
			...
			cin>>x>>w;
			x--;
			ll now=seg.get(steps[x],steps[x]);
			seg.update(steps[x],now+w);
			costsum+=w;
		}else{
			ll y;
			cin>>y;
			y--;
			ll tochild;
			if(steps[u[y]]>steps[v[y]])tochild=u[y];
			else tochild=v[y];

			ll childcost=seg.get(steps[tochild],endes[tochild]);
			ll ans=abs(childcost*2-costsum);
			cout<<ans<<endl;
		}
	}
	return 0;
