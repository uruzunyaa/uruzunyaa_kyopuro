...
int solve() {
    ...
    vll nextPlayers(n);
    rep(i, n) nextPlayers[i] = (i + 1) % n;

    ll gonePlayerCnt = 0;
    vll gonePlayers(n);

    vector<map<ll, ll>> cards(n, map<ll, ll>());
    rep(i, n) {
        ll c1, c2;
        cin >> c1 >> c2;
		if (c1 == c2) {
	...
	ll cur = 0;
	ll ans = 0;
    while (gonePlayerCnt < n) {
		while(gonePlayers[cur]) { cur++; cur %= n; }

		ll next = (cur + 1) % n;
		while(gonePlayers[next]) { next++; next %= n; }
		
		ll minCard = cards[cur].begin()->first;
		cards[cur][minCard]--;
		if (cards[cur][minCard] == 0) cards[cur].erase(minCard);
		if (cards[cur].empty()) {
			gonePlayers[cur] = 1;
			gonePlayerCnt++;
		}
		...
		cards[next][minCard]++;
		if (cards[next][minCard] == 2) cards[next].erase(minCard);
		if (cards[next].empty()) {
			gonePlayers[next] = 1;
			gonePlayerCnt++;
		}
