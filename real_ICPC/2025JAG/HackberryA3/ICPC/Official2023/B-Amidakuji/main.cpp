...
Result amidakuji(ll start, ll goal, vll &edge, P additional) {
	vector<P> path;

    auto move =
        [&](ll x) -> bool {
            if (start == x) { start = x + 1; return true; }
            else if (start == x + 1) { start = x; return true; }
			return false;
        };

    rep(i, edge.size()) {
        if (i == additional.first) move(additional.second);
		
		if(move(edge[i])) {
			path.emplace_back(i, start);
		}
    }
	if (edge.size() == additional.first) move(additional.second);
...
int solve() {
	...
	auto first = amidakuji(p, q, edge, {-1, -1});
	if (first.goal){
		cout << "OK" << endl;
		return 0;
	}
	...
	ll now = 0;
	rep(i, m + 1) {
		if (now < first.path.size() && first.path[now].first < i) {
			x = first.path[now].second;
			now++;
		}
		...
		loop(j, -1, 1) {
			...
			auto result = amidakuji(p, q, edge, {newY, newX});
			if (result.goal) {
				cout << newX << " " << newY << endl;
				return 0;
			}
