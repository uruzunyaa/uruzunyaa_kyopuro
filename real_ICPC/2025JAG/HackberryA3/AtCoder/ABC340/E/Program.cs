...
class Program
{
	static void Main(string[] args)
	{
		...
		var seg = LazySegTree<long>.RangeAddSumQuery(n);
		for (int i = 0; i < n; i++) seg[i] = cin;
		seg.Build();
		...
		for (int i = 0; i < m; i++)
		{
			...
			seg.Update(b, 0);
			// 全体に対する個数
			seg.Update(0, n, count / n);
			// 余り
			long left = count % n;
			if(n < b + 1 + left)
			{
				seg.Update(b + 1, n, 1);
				seg.Update(0, (int)(b + 1 + left - n), 1);
			}
			else
			{
				seg.Update(b + 1, (int)(b + 1 + left), 1);
			}
		}

		foreach (var v in seg[0..n]) Console.Write(v + " ");
	}
}
