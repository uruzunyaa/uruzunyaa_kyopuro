// #確率の対称性
// n個の玉があり、１番左の玉は黒、それ以外は白である。２つの玉を当確率で選び、入れ替える操作をk回行う。そのとき、黒い玉がある位置の期待値を求めよ。
#include <bits/stdc++.h>
using namespace std;
using ll = long long;
using vll = vector<ll>;
...
int solve() {
	ll n, k; cin >> n >> k;
	// 各遷移で、黒い玉が選ばれる確率は、2/n^2 * nで、選ばれない確率は、n^2 - 2n/n^2 = (n - 2)/n
	ll p = (n - 2) % MOD * inv(n, MOD) % MOD;
	// k回で、１回も選ばれない確率 = p^k
	p = pow(p, k, MOD);
	// １回以上選ばれる確率 = 1 - p^k
	// 1から引くと、負になる可能性があるので、MODを足してから引く
	ll q = (1 + MOD - p) % MOD;

	// 期待値 = 等差数列の和の平均 = (1 + n) * n / 2 / n
	ll E = (1 + n) * n % MOD * inv(2, MOD) % MOD * inv(n, MOD) % MOD;

	cout << (p + q * E % MOD) % MOD << endl;
