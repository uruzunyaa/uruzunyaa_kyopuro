...
int solve() {
	...
	// 座標圧縮
	set<ll> compressed;
	rep(i, n) compressed.insert(a[i]);
	rep(i, q) compressed.insert(queries[i].second.first);
	map<ll, ll> compressedMap;
	ll idx = 0;
	for (auto &c : compressed) compressedMap[c] = idx++;

	segtree<ll> seg(compressed.size(), 0, [](ll a, ll b) { return max(a, b); });
	...
	rep(i, q) {
		...
		ll idx = queries[i].second.second;

		while (ai < r) {
			ll c = compressedMap[a[ai]];
			ll maxVal = seg.query(0, c);

			if (seg[c] < maxVal + 1) {
				seg.update(c, maxVal + 1);
			}

			++ai;
		}

		ans[idx] = seg.query(0, compressedMap[x] + 1);
	}
