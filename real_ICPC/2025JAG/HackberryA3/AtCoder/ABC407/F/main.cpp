// imos法 区間 遅延セグメント木
// 自分を含む区間で自分が最大値になるような区間の数は左に伸ばせる数（始点になる数）と右に伸ばせる数（終点になる数）の掛け算で求まる。
// 区間の長さkを分類しようとしたとき、min(左に伸ばせる数, k) * min(右に伸ばせる数, k)を計算していくと階段状になっているので、遅延セグ木でimos法を使う。
#include <bits/stdc++.h>
using namespace std;
using ll = long long;
...
int solve() {
	...
	lazy_segtree<ll> imos = lazy_segtree<ll>::RangeAddSumQuery(n + 1);
	segtree<ll> seg = segtree<ll>::RangeMaximumQuery(n);
	rep(i, n) {
		seg[i] = a[i];
	}
	seg.build();
	...
	rep(i, n) {
		ll mn = min(before[i], after[i]);
		ll mx = max(before[i], after[i]);
		imos.update(0, min(mn + 1, n + 1), a[i]);
		imos.update(mx + 1, min(mx + mn + 1, n + 1), -a[i]);
	}
