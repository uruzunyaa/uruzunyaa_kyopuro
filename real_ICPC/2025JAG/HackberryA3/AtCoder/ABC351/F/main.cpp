...
struct Monoid {
	...
	ll cnt;
	Monoid() : sum(0), cnt(0) {}
	Monoid(ll sum, ll cnt) : sum(sum), cnt(cnt) {}
};
...
int solve() {
	...
	set<ll> compress;
	rep(i, n) compress.insert(a[i]);
	map<ll, ll> compressMap;
	ll idx = 0;
	for (ll x : compress) compressMap[x] = idx++;

	segtree<Monoid> seg = segtree<Monoid>(compress.size(), Monoid(), [](Monoid a, Monoid b) {
		return Monoid(a.sum + b.sum, a.cnt + b.cnt);
	});
	ll ans = 0;
	rep(j, n) {
		Monoid res = seg.query(0, compressMap[a[j]] + 1);
		ans += a[j] * res.cnt - res.sum;

		Monoid newMonoid = seg[compressMap[a[j]]];
		newMonoid.sum += a[j];
		newMonoid.cnt++;
		seg.update(compressMap[a[j]], newMonoid);
	}
