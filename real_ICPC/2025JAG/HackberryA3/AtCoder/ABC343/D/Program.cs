...
class Program
{
	static void Main(string[] args)
	{
		...
		Dictionary<long, int> kind = new ();
		kind[0] = n;

		for (int i = 0; i < t; i++)
		{
			int a = cin - 1, b = cin;
			if(kind.ContainsKey(scores[a])) kind[scores[a]]--;
			if (kind[scores[a]] == 0) kind.Remove(scores[a]);

			scores[a] += b;
			if (!kind.ContainsKey(scores[a])) kind[scores[a]] = 0;
			kind[scores[a]]++;
