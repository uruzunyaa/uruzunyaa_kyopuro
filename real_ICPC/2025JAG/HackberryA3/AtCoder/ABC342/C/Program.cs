...
class Program
{
	static void Main(string[] args)
	{
		...
		Dictionary<char, List<int>> charmap = new Dictionary<char, List<int>>();
		for (char c = 'a'; c <= 'z'; c++) charmap[c] = new List<int>(n);
		for (int i = 0; i < n; i++) charmap[chars[i]].Add(i);
		int q = cin;
		(char c, char d)[] queries = new (char c, char d)[q];
		for (int i = 0; i < q; i++) queries[i] = (cin, cin);
		...
		for (int i = 0; i < 26; i++)
		{
			...
			map[(char)('a' + i)] = current;
		}
		foreach (var item in map)
		{
			List<int> indexes = charmap[item.Key];
			foreach (var index in indexes)
			{
				chars[index] = item.Value;
			}
		}
