...
struct DijkstraResult
{
    ...
    DijkstraResult(const vector<ll> &dist, const vector<int> &prev)
        : dist(dist), prev(prev) {}
    ...
    vector<int> restore(const int goal) const {
        vector<int> path;
        for (int now = goal; now != -1; now = prev[now])
            path.push_back(now);
 ...
 * @return DijkstraGridResult dist[x]
 * にスタート地点からxまでの最短距離が格納される @n restore(long long
 * goal)で経路復元できる
 */
template <class T>
DijkstraResult dijkstra(const vector<vector<Edge<T>>> &G, const int start,
                        const T inf) {
...
int solve() {
    ...
    Graph<schedule> g(n, vector<Edge<schedule>>());
    rep(i, m) {
        ll l, d, k, c, a, b;
        cin >> l >> d >> k >> c >> a >> b;
        --a, --b;
        g[b].push_back(Edge<schedule>(b, a, {l, d, k, a, b, c}));
    }
    ...
    q.push({INF, n - 1});
    while (!q.empty()) {
        auto [nowTime, nowSta] = q.top();
        q.pop();
		if (lastTrain[nowSta] != -1 && lastTrain[nowSta] > nowTime) continue;

		for (const Edge<schedule> &e : g[nowSta]) {
			auto [l, d, k, a, b, c] = e.val;
			ll leaveTime = nowTime - c;
			if (leaveTime < l) continue;

			ll lastTime = min((leaveTime - l) / d, k - 1) * d + l;
			if (lastTrain[a] != -1 && lastTrain[a] > lastTime) continue;
			lastTrain[a] = lastTime;
			q.push({lastTime, a});
		}
	}

	rep(i, n - 1) cout << (lastTrain[i] == -1 ? "Unreachable" : to_string(lastTrain[i])) << endl;
