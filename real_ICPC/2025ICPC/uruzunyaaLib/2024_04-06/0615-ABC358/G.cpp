...
int main(){
	ll h,w,k,sx,sy;
	cin>>h>>w>>k>>sx>>sy,sx--,sy--;

	vvl a(h,vl(w));
	rep(i,h)rep(j,w)cin>>a[i][j];

	vector<vector<pair<ll,ll>>> cost(h,vector<pair<ll,ll>>(w,{inf,0}));
	cost[sx][sy]={0,0};
	...
	while(!bfs.empty()){
		...
		bfs.pop();
		//行動回数が限界突破したらout
		if(k<cost[x][y].first)continue;
		rep(i,4){
			//枠外に行くならcontinue
			if(x+dx[i]<0||x+dx[i]>=h||y+dy[i]<0||y+dy[i]>=w)continue;
			pair<ll,ll> tmp = {cost[x][y].first+1,cost[x][y].second-a[x+dx[i]][y+dy[i]]};
			if(cost[x+dx[i]][y+dy[i]]>tmp){
				cost[x+dx[i]][y+dy[i]]=tmp;
	...
	//各マスで残り回数づっと留まる場合を調査
	rep(i,h)rep(j,w){
		if(k<cost[i][j].first)continue;
		ans=max(ans,a[i][j]*(k-cost[i][j].first)-cost[i][j].second);
	}
	cout<<ans<<endl;
