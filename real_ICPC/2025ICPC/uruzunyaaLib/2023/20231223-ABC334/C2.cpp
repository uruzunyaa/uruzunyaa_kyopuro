...
int main(){
	...
	if(k==1){
		...
		//初期値の設定
		hidari[0]=a[1]-a[0];
		hidari[k-2]=a[k-1]-a[k-2];
		for(ll i=2;i<k-1;i+=2){
			hidari[i]=a[i+1]-a[i]+hidari[i-2];
			hidari[k-i-2]=a[k-i-1]-a[k-i-2]+hidari[k-i];
		}
		//累積和を利用して、捨てる物を全探索
		ll mn=inf;
		//端っこを先に探索
		mn=min(mn,hidari[k-3]);
		mn=min(mn,hidari[1]);
		for(ll i=2;i<=k-3;i+=2){
			mn=min(mn,hidari[i-2]+hidari[i+1]);
		}
		ans=mn;
	}else{
		rep(i,k/2){
