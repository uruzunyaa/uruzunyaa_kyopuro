...
int main(){
	...
	//i人の場合について
	loop(i,2,n){
		ll inv=power_mod(power_mod(2,i)-1,mod-2);
		//人iの答えを計算
		rep(j,i-1)dp[i][i-1]+=(power_mod(2,j+1)*dp[i-1][j]%mod)*inv%mod;
		dp[i][i-1]%=mod;

		//次の人の答えを差分で計算
		for(ll j=i-2;j>=0;j--){
			dp[i][j]+=dp[i][j+1]+mod-(dp[i-1][j]*power_mod(2,i-1)%mod)*inv%mod;
			dp[i][j]%=mod;
			dp[i][j]*=2;
			dp[i][j]%=mod;
			dp[i][j]+=dp[i-1][j]*inv%mod;
			dp[i][j]%=mod;
		}
	}
	rep(i,n)cout<<dp[n][i]<<" ";
