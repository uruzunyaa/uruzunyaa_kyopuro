...
#define inf 400000000000000000LL
...
ll dist(ll x,ll y){
	...
	while(!bfs.empty()){
		ll tmp=bfs.front();
		bfs.pop();
		//if(ans[tmp]!=inf)continue;
		rep(i,g[tmp].size()){
			if(ans[g[tmp][i]]!=inf)continue;
			ans[g[tmp][i]]=ans[tmp]+1;
			bfs.push(g[tmp][i]);
		}
	}
	//vdbg(ans);
...
int main(){
	...
	ll ans=min(dist(0,n-2)+dist(n-2,n-1)+dist(n-1,0),dist(0,n-1)+dist(n-1,n-2)+dist(n-2,0));
	if(ans>=inf)ans=-1;
	cout<<ans<<endl;
