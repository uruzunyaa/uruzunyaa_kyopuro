...
int main(){
    ...
    vl s(n);
    vl e(n);
    rep(i,n)cin>>s[i]>>e[i],e[i]+=s[i];

    vector<pair<ll,ll>> se(n);

    rep(i,n)se[i]=make_pair(s[i],e[i]);
    ...
    while(1){
        ...
        //何も起きない場合、何か起きる所まで時間を進める。
        if(t<se[indexS].first&&count.size()==0)t=se[indexS].first;

        //今の時間に印字機の範囲に入るやつを追加
        while(t==se[indexS].first&&indexS<n){
            count[se[indexS].second]++;
            indexS++;
        }
        ...
        if(count.size()!=0){
            count.begin()->second--;
            if(count.begin()->second==0)count.erase(count.begin());
            ans++;
        }

        //今の時間に印字機から出る奴を出す
        if(t==count.begin()->first){
            count.erase(count.begin());
        }

        //全てが終わったら、処理を終える
        if(count.size()==0&&indexS==n)break;
    }
    cout<<ans<<endl;
    return 0;
}