...
// Union-Find
struct UnionFind {
	vector<int> par, siz ,a,b;
	UnionFind(int n) : par(n, -1) , siz(n, 1) ,a(n,0),b(n,0){ }
	// 根を求める
	int root(int x) {
		if (par[x] == -1) return x;
...
int main(){
	...
	ll ans=0;
	rep(i,m){
		if(uf.issame(g[i][1],g[i][2]))continue;
		uf.unite(g[i][1],g[i][2]);
		ans+=uf.match(g[i][1])*g[i][0];
	}
	cout<<ans<<endl;
	return 0;
}
