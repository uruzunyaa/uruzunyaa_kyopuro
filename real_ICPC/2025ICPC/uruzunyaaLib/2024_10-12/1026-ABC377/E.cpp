...
int main(){
	...
	rep(i,n)cin>>p[i],p[i]--;
	vvl cycle(n);
	pair<ll,ll> fls={-inf,-inf};
	vector<pair<ll,ll>> kiroku(n,fls);
	//kiroku[i]={リーダーノード,その何個目}

	rep(i,n){
		if(kiroku[i]!=fls)continue;
		ll tmp=i;
		rep(j,inf){
			kiroku[tmp]={i,j};
			cycle[i].push_back(tmp);
			tmp=p[tmp];
			if(tmp==i)break;
		}
	}
	rep(i,n){
		ll mod=cycle[kiroku[i].first].size();
		ll tmp=power_mod(2,k,mod)+kiroku[i].second;
		tmp%=mod;
		ll ans=cycle[kiroku[i].first][tmp];
		cout<<ans+1<<" ";
	}
	cout<<endl;
