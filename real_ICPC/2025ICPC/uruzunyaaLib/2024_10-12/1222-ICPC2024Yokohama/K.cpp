...
int main(){
	...
	rep(i,n){
		...
		a[i]={-__popcount(tmp),i,tmp};
	}
	sort(a.begin(),a.end());

	vector<bool> b(1<<m,false);

	ll cnt=0;
	vvl erase;
	rep(i,n){
		if(b[a[i][2]])continue;
		if(a[i][2]==pm2-1&&cnt==0){
			erase.push_back(a[i]);
			cnt++;
			continue;
		}
	...
	rep(i,erase.size()){
		rep(j,i){
			if((erase[i][2]|erase[j][2])==pm2-1){
				ans.push_back({erase[i][0]+erase[j][0],min(erase[j][1],erase[i][1]),max(erase[j][1],erase[i][1])});
			}
		}
	}
	sort(ans.begin(),ans.end());
	if(ans.size()!=0)cout<<ans[0][1]+1<<" "<<ans[0][2]+1<<endl;
	else cout<<"No"<<endl;
	return 0;
}
