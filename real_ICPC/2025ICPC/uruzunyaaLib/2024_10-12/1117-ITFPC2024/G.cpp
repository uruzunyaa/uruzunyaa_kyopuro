...
int main(){
	...
	rep(i,n){
		cin>>a[i];
		if(a[i]<=5000)gr[i]=g[a[i]];
		else gr[i]=g[a[i]%12+84];
		xr^=gr[i];
	}
	ll ans=0;
	if(xr!=0)ans+=2;
	...
	//どちらも最初の山の石を食べれない場合勝敗だけで確定
	if(a[0]<=2){
		cout<<ans<<endl;
		return 0;
	}

	//後手が勝つ場合、先手が即おいしい石を食べる
	if(xr==0){
		cout<<-1<<endl;
		return 0;
	}

	//先手が、山1の石を最初に食べた上で勝利出来るか判定する。
	xr^=gr[0];
	if(a[0]>5000)a[0]%=1200+1200;
	...
	//先手が後手が美味しい石を食べれないように妨害した上で勝利出来るか判定する。
	if((g[1]^g[a[0]-1])==xr){
		cout<<2<<endl;
		return 0;
	}
