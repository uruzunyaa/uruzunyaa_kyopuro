...
int main(){
	...
	loop(i,1,t.size()-1){
		...
		//s.insert(t[i].first);
		while(1){
			if(k+1!=t.size()&&t[k+1].second==2&&!s.count(t[k+1].first)){
				k++;
				s.insert(t[k].first);
			}else break;
		}
		if(t[i].second>=2){
			...
			ll tmp=0;
			if(!s.count(t[i].first))tmp++;
			if(k+1!=t.size()&&t[k+1].second>=2&&!s.count(t[k+1].first)&&t[i].first!=t[k+1].first)tmp++;
			ans=max(ans,(k-i+tmp)*2);
		}
	}
	cout<<ans<<endl;
	return 0;
