...
int main(){
	...
	string s;
	cin>>n>>q>>s;
	vl one(n+1,0),two(n+1,0),thla(n+1,0);
	...
	rep(z,q){
		...
		ll ansmn=0;
		while(ansmx!=ansmn){
			ll ansmid=(ansmx+ansmn+1)/2;
			ll oneindmx=r;
			ll oneindmn=l;
			while(oneindmx>oneindmn){
				ll indmid=(oneindmx+oneindmn)/2;
				if(one[indmid]-one[l-1]<ansmid)oneindmn=indmid+1;
				else oneindmx=indmid;
			}

			ll thlaindmx=r;
			ll thlaindmn=min(oneindmn+1,r);
			while(thlaindmx>thlaindmn){
				ll indmid=(thlaindmx+thlaindmn)/2;
				if(thla[indmid]-thla[oneindmn]<1)thlaindmn=indmid+1;
				else thlaindmx=indmid;
			}

			ll twoindmx=r;
			ll twoindmn=min(thlaindmn+1,r);
			while(twoindmx>twoindmn){
				ll indmid=(twoindmx+twoindmn)/2;
				if(two[indmid]-two[thlaindmn]<ansmid)twoindmn=indmid+1;
				else twoindmx=indmid;
			}
			
			//cout<<oneindmx<<" "<<thlaindmn<<" "<<twoindmn<<endl;
			
			if(one[oneindmn]-one[l-1]==ansmid&&thla[thlaindmn]-thla[oneindmn]==1&&two[twoindmn]-two[thlaindmn]==ansmid){
				ansmn=ansmid;
			}else{
				ansmx=ansmid-1;
			}
		}
		if(thla[r]-thla[l-1]!=0)cout<<ansmn*2+1<<endl;
		else cout<<0<<endl;
	}
	return 0;
}
