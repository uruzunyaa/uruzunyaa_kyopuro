...
int main(){
	...
	for(ll i = n % 2; i + 1 < n; i += 2){
		...
		if (!rs.empty() && !rb.empty() && *rs.rbegin() > *rb.begin()) {
			ll tmpb = *rs.rbegin();
			ll tmps = *rb.begin();
			rs.erase(prev(rs.end()));
			rb.erase(rb.begin());
			rs.insert(tmps);
			rb.insert(tmpb);
			right += (tmpb - tmps) * 2;
	...
	for(ll i = 2; i < n; i += 2){
		// 左側の処理
		ll small = min(a[i - 1], a[i - 2]);
		ll big = max(a[i - 1], a[i - 2]);
		ls.insert(small);
		lb.insert(big);
		left += big - small;

		if (!ls.empty() && !lb.empty() && *ls.rbegin() > *lb.begin()) {
			ll tmpb = *ls.rbegin();
			ll tmps = *lb.begin();
