...
int main(){
	...
	ll cnt=0;
	rep(i,n){
		if(s[i]=='(')cnt++,seg.apply(i,i+n,1);
		else cnt--,seg.apply(i,i+n,-1);
	}
	ll ans=inf;
	rep(i,n){
		//開き括弧に変える個数。
		ll hirakinisuru=0;
		if(seg.prod(i,i+n)<0)
		hirakinisuru=(-seg.prod(i,i+n)+1)/2;
		//開き括弧が何個多いか。
		ll tmp=cnt+hirakinisuru*2;

		ll change= hirakinisuru*m + (tmp/2)*m;

		ans=min(ans,change+i*r);

		if(s[i]=='(')seg.apply(i,i+n,-1),seg.apply(i+n,2*n,+1);
		else seg.apply(i,i+n,+1),seg.apply(i+n,2*n,-1);
	}
	cout<<ans<<endl;
	return 0;
}
