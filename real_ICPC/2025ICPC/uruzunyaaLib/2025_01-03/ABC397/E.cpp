...
int main(){
	...
	while(!q.empty()){
		...
		if(dp[tmp].size()>=3){
			cout<<"No"<<endl;
			return 0;
		}else if(dp[tmp].size()==2){
			if(dp[tmp][0]+dp[tmp][1]+1!=k){
				cout<<"No"<<endl;
				return 0;
			}
		}else if(dp[tmp].size()==1){
			if(dp[tmp][0]+1!=k){
				dp[*g[tmp].begin()].push_back(dp[tmp][0]+1);
			}
		}else{
			dp[*g[tmp].begin()].push_back(1);
		}
