...
int main(){
	...
	priority_queue<vl> p;
	set<vl> s;
	vl q={a[0]*b[0]+b[0]*c[0]+c[0]*a[0],0,0,0};
	p.push(q);
	rep(i,k){
		ll ans=p.top().at(0);
		ll ia=p.top().at(1);
		...
		if(ia!=n-1){
			q={a[ia+1]*b[ib]+b[ib]*c[ic]+c[ic]*a[ia+1],ia+1,ib,ic};
			p.push(q);
		}
		if(ib!=n-1){
			q={a[ia]*b[ib+1]+b[ib+1]*c[ic]+c[ic]*a[ia],ia,ib+1,ic};
			p.push(q);
		}
		if(ic!=n-1){
			q={a[ia]*b[ib]+b[ib]*c[ic+1]+c[ic+1]*a[ia],ia,ib,ic+1};
			p.push(q);
		}
	}
	return 0;
