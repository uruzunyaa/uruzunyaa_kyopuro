...
int main(){
	ll n,size;
	cin>>n>>size;
	vector<vector<vector<map<ll,ll>>>> p(100,vector<vector<map<ll,ll>>>(100,vector<map<ll,ll>>(100)));
	rep(i,n){
		ll a,b,c,h;
		cin>>a>>b>>c>>h;
		p[a][b][c][h]++;
	}
	vector<vvl> imos(100,vvl(102,vl(102,0)));
	rep(i,100){
		rep(j,100){
			rep(k,100){
				if(p[i][j][k].size()==0)continue;
				ll mx=p[i][j][k].rend()->first;
				rloop(l,mx,2){
					p[i][j][k][l-1]+=p[i][j][k][l];
				}
				loop(l,1,mx){
					imos[k+l-1][i+l][j+l]+=p[i][j][k][l];
					imos[k+l-1][i-l+1][j-l+1]+=p[i][j][k][l];
					imos[k+l-1][i-l+1][j+l]-=p[i][j][k][l];
					imos[k+l-1][i+l][j-l+1]-=p[i][j][k][l];
				}
			}
		}
	}
	ll ans=0;
	vector<vvl> hukugen(100,vvl(102,vl(102,0)));
	rep(i,100){
		rloop(j,100,0){
			ll sum=0;
			rloop(k,100,0){
				sum+=imos[i][j][k];
				hukugen[i][j][k]=hukugen[i][j+1][k]+sum;
				if(hukugen[i][j][k]>=size)ans++;
			}
		}
	}
	cout<<ans<<endl;
