		...
		rep(i,x.size()+1){
			rep(j,y.size()+1){
				...
				if(i!=x.size()&&j!=y.size()){
					if(xd[i]==y[j])dp[i+1][j+1]=min(dp[i][j],dp[i+1][j+1]);
					else dp[i+1][j+1]=min(dp[i][j]+s,dp[i+1][j+1]);
				}
				//追加の遷移
				if(j!=y.size()){
				...
				//削除の遷移
				if(i!=x.size()){
					if(i+k>=x.size())dp[i+1][j]=min(dp[i][j]+e-r,dp[i+1][j]);
					else dp[i+1][j]=min(dp[i][j]+e,dp[i+1][j]);
				}
			}
		}
		//デバッグ
		//cout<<xd<<endl;
		//if(ans>dp[x.size()][y.size()])cout<<k<<endl;
