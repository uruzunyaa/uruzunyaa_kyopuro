...
ll solve(){
	...
	vector<set<ll>> g(n);
	vector<double> p(n);
	vector<bool> kisyou(n,false);
	vector<bool> ziriki(n,true);
	UnionFind uf(n);
	double ans=1;
	...
	rep(i,n){
		...
		while(!bfs.empty()){
			...
			kisyou[j]=true;
			bfs.pop();
			for(const auto &val:g[j]){
				if(kisyou[val])continue;
				bfs.push(val);
			}
		}
	}
