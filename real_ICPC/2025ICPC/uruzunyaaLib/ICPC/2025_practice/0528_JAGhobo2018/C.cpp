...
ll solve(){
	...
	sort(rw.rbegin(),rw.rend());

	if(sum-rw[0].first>=rw[0].first){
		rep(i,n)sum+=rw[i].second;
		cout<<sum<<endl;
		return 0;
	}

	ll x=rw[0].first*2-sum;
	vector<vector<bool>> dp(n+1,vector<bool>(x+1,false));
	dp[1][0]=true;
	loop(i,1,n-1){
		dp[i+1]=dp[i];
		loop(j,rw[i].second,x){
			if(dp[i][j-rw[i].second])dp[i+1][j]=true;
		}
	}
	ll b=0;
	loop(i,0,x)if(dp[n][i])b=i;
