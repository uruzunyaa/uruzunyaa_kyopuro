...
int main(){
	...
	if(c>0){
		ll mx=max_element(imos.begin(),imos.end())-imos.begin();
		ll mn=min_element(imos.begin(),imos.begin()+mx)-imos.begin();
		mn--;
		ll ans=0;
		rep(i,n){
			if(mn<i&&i<mx)ans+=a[i]*c;
			else ans+=a[i];
		}
		cout<<ans<<endl;
		return 0;
	}
	ll mn=min_element(imos.begin(),imos.end())-imos.begin();
	ll mx=max_element(imos.begin(),imos.begin()+mn)-imos.begin();
	mx--;
	ll ans=0;
	rep(i,n){
		if(mx<i&&i<mn)ans+=a[i]*c;
		else ans+=a[i];
	}
	cout<<ans<<endl;
