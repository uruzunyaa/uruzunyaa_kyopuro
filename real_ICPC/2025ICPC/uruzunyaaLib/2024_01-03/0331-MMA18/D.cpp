...
int main(){
	...
	sort(a.begin(),a.end());
	vl ten(n),nine(n),seven(n),five(n);

	rep(i,n){
		ten[i]=power_mod(10,a[i],p);
		nine[i]=power_mod(9,a[i],p);
		seven[i]=power_mod(7,a[i],p);
		five[i]=power_mod(5,a[i],p);
	}
	ll ans=0;
	rep(i,n)rep(j,i)rep(k,j)rep(l,k){
		if((five[i]+seven[j]+nine[k]+ten[l])%p==q)ans++;
	}
	cout<<ans<<endl;
	return 0;
}
