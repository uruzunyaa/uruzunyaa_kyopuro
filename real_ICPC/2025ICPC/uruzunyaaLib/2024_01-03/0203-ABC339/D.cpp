...
int main(){
	...
	rep(i,n)cin>>ss[i];
	n+=2;
	vector<string> s=pad(ss,'#');

	queue<ll> bfs;
	vl k(n*n*n*n,inf);
	ll tmp=inf;
	rep(i,n)rep(j,n)if(s[i][j]=='P'){
		if(tmp==inf)tmp=i*n+j;
		else{
			k[n*n*n*i+n*n*j+tmp]=0;
			bfs.push(n*n*n*i+n*n*j+tmp);
		}
	}
	while(!bfs.empty()){
		ll row=bfs.front();
		...
		rep(i,4){
			...
			ll nqx=qx+dx[i];
			ll nqy=qy+dy[i];
			if(s[px+dx[i]][py+dy[i]]=='#')npx=px,npy=py;
			if(s[qx+dx[i]][qy+dy[i]]=='#')nqx=qx,nqy=qy;
			ll nrow=npx*n*n*n+npy*n*n+nqx*n+nqy;
			if(k[nrow]==inf){
				k[nrow]=k[row]+1;
				bfs.push(nrow);
			}
