...
int main(){
	...
	loop(i,1,n)cin>>a[i]>>b[i]>>c[i]>>d[i];

	//m[i][j]=頂点iから頂点jに移動するコスト
	vvl m(n+2,vl(n+2,inf));
	rep(i,n+2)m[i][i]=0;
	...
	loop(i,1,n){
		loop(j,1,n){
			m[i][j]=abs(c[i]-a[j])+abs(d[i]-b[j])+1;
		}
	}
	n+=2;
	...
	rep(i,n){
		ll mn=n-1;
		rep(j,n){
			if(check[j]&&ans[mn]>=ans[j])mn=j;
		}
		check[mn]=false;
		rep(j,n){
			ans[j]=min(ans[j],ans[mn]+m[mn][j]);
		}
	}
	cout<<ans[n-1]<<endl;
