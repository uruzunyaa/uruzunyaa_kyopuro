...
//notmax[i][l]=i番目がダメで、l番目を左端とした時、最も近くにある右の場所。
vvl notmax(500,vl(500,inf));
vvl dp(500,vl(500,-1));
vvl ncr(500,vl(500));
//
ll ans(ll l,ll r){
	...
	ll res=0;
	loop(i,l,r){
		if(notmax[i][l]<=r)continue;
		ll tmp=ans(l,i-1)*ans(i+1,r);
		tmp%=mod;
		tmp*=ncr[r-l][i-l];
		tmp%=mod;
		res+=tmp,res%=mod;
...
int main(){
	...
	rep(i,m){
		ll l,r,x;
		cin>>l>>r>>x,l--,r--,x--;
		loop(j,0,l)notmax[x][j]=min(notmax[x][j],r);
	}
	cout<<ans(0,n-1)<<endl;
	//vvdbg(dp);
	return 0;
