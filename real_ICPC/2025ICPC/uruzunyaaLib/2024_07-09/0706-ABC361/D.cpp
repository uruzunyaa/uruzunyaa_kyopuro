...
int main(){
	...
	ll bit=power(3,n)*8;
	rep(i,n){
		if(s[i]=='W')b+=power(3,i);
		if(t[i]=='W')bit+=power(3,i);
	}
	ok[b]=0;
	queue<ll> bfs;
	bfs.push(b);

	while(!bfs.empty()&&!ok.count(bit)){
		ll c=bfs.front();
		bfs.pop();
		ll x=0;
		rep(i,n+1)if((c/power(3,i))%3==2){
			x=i;
			break;
		}

		rep(i,n+1){
			if(i-1==x||i==x||i+1==x)continue;
			ll tmp=c/power(3,i)%9;
			ll nw=c+8*power(3,i)-tmp*power(3,i)+tmp*power(3,x)-8*power(3,x);
			if(!ok.count(nw)){
				ok[nw]=ok[c]+1;
				bfs.push(nw);
			}
