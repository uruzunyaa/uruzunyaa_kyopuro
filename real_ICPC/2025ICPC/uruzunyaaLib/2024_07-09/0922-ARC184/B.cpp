...
int main(){
	...
	ll ans=0;
	while(cnt.size()>0){
		ll tmp=cnt.rbegin()->first;
		cout<<tmp<<" "<<cnt[tmp]<<endl;
		ans+=cnt[tmp];
		if(tmp%3==0){
			ll three=1;
			while(tmp%power(3,three)==0){
				ll two=0;
				while(tmp%power(2,two)==0){
					cnt[tmp/power(2,two)/power(3,three)]-=cnt[tmp]*3;
					if(cnt[tmp/power(2,two)/power(3,three)]<=0)cnt.erase(tmp/power(2,two)/power(3,three));
					two++;
				}
				cnt[tmp*2/power(3,three)]-=cnt[tmp];
				if(cnt[tmp*2/power(3,three)]<=0)cnt.erase(tmp*2/power(3,three));
				three++;
			}
			ll two=1;
			while(tmp%power(2,two)==0){
				cnt[tmp/power(2,two)]-=cnt[tmp];
				if(cnt[tmp/power(2,two)]<=0)cnt.erase(tmp/power(2,two));
				two++;
			}
			cnt.erase(tmp);
		}else if(tmp%2==0){
