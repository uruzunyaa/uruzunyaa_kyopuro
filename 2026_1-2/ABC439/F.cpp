...
pair<ll,ll> sm(pair<ll,ll> a,pair<ll,ll> b){
...
int main(){
	...
	SegTree<pair<ll,ll>> seg(v,sm,{0,0});

	SegTree<ll> seg2(vl(n,0),sm2,0);

	ll ans=0;
	rep(i,n){
		pair<ll,ll> small=seg.get(0,p[i]);
		pair<ll,ll> big=seg.get(p[i],n-1);
		pair<ll,ll> tmp={big.first+big.second,small.first+small.second};
		
		ans+=tmp.first;
		ans%=mod;

		tmp.second+=seg2.get(0,p[i]);
		tmp.first%=mod;
		tmp.second%=mod;
		seg.update(p[i],tmp);
		seg2.update(p[i],seg2.get(p[i],p[i])+1);
