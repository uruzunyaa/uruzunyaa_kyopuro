...
int main(){
	...
	vvl dp(2*n,vl(2*n,0));
	//左上から、その座標まででの黒の個数を2週分計算。
	loop(i,1,n+n-1)loop(j,1,n+n-1){
		dp[i][j]=dp[i][j-1]+dp[i-1][j]-dp[i-1][j-1];
		if(p[(i-1)%n][(j-1)%n]=='B')dp[i][j]++;
	}
	rep(z,q){
		ll a,b,c,d;
		cin>>a>>b>>c>>d,c++,d++;
		...
		//右下側でPを丸々含む場合、その数を数える。
		ans+=((c-a)/n)*((d-b)/n)*dp[n][n];
		...
		//残りの上のスペースを数える
		ans+=(dp[e][n]-dp[a][n])*((d-b)/n);

		//残りの左のスペースを数える
		ans+=(dp[n][f]-dp[n][b])*((c-a)/n);

		//左上の残り領域を数える。
		ans+=dp[e][f]-dp[a][f]-dp[e][b]+dp[a][b];
