...
string convertBase(const string &num_str, ll from_base, ll to_base) {
    ll num = 0;
...
int main(){
	...
	rep(i,t){
		ll n,k;
		cin>>n>>k;
		string base=convertBase(to_string(n),10,3);
		ll sum=0;
		rep(i,base.size()) sum+=base[i]-'0';
		if(sum<=k&&k<=n&&(sum-k)%2==0){
			cout<<"Yes"<<endl;
		}else{
			cout<<"No"<<endl;
		}
