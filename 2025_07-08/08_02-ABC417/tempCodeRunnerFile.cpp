...
ll memo(ll i,ll x){
	...
	if(x>500){
		ll sk=lower_bound(sums.begin(),sums.end(),x+sums[i]-500)-sums.begin();
		return memo(sk,x+sums[i]-sums[sk]);
	}
	if(dp[i][x]!=-inf){
		return dp[i][x];
	}
	ll tmp;
	if(x<=p[i])tmp=memo(i+1,x+a[i]);
	else tmp=memo(i+1,max(x-b[i],0LL));
	dp[i][x]=tmp;
	return tmp;
}
...
int main(){
	...
	rep(i,n){
		...
		a.push_back(aa);
		b.push_back(bb);
		sums.push_back(sums.back()+bb);
	}
	dp=vvl(n+1,vl(501,-inf));
