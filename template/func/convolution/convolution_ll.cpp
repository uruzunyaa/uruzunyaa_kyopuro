...
// safe_mod (ACL と互換な動き)
inline ll safe_mod_ll(ll x, ll m){
    x %= m;
    if (x < 0) x += m;
    return x;
}
...
inline vector<ll> convolution_ll(const vector<ll> &a, const vector<ll> &b){
    ...
    // 3つの NTT-friendly 素数
    static constexpr unsigned ll MOD1 = 754974721;  // g = 11
    static constexpr unsigned ll MOD2 = 167772161;  // g = 3
    static constexpr unsigned ll MOD3 = 469762049;  // g = 3
    static constexpr unsigned ll M2M3 = MOD2 * MOD3;
    static constexpr unsigned ll M1M3 = MOD1 * MOD3;
    static constexpr unsigned ll M1M2 = MOD1 * MOD2;
    static constexpr unsigned ll M123 = MOD1 * MOD2 * MOD3;

    static const unsigned ll i1 =
        (unsigned ll)inv_mod_any((ll)(M2M3 % MOD1), (ll)MOD1);
    static const unsigned ll i2 =
        (unsigned ll)inv_mod_any((ll)(M1M3 % MOD2), (ll)MOD2);
    static const unsigned ll i3 =
        (unsigned ll)inv_mod_any((ll)(M1M2 % MOD3), (ll)MOD3);
    ...
    for (int i = 0; i < n + m - 1; i++){
        ...
        ll diff =
            (ll)c1[i] - safe_mod_ll((ll)x, (ll)MOD1);
        if (diff < 0) diff += MOD1;

        static constexpr unsigned ll offset[5] = {
