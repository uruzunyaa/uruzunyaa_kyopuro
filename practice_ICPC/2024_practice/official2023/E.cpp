...
ll solve(){
	...
	vector<pair<ll,ll>> yx(n);
	rep(i,n)cin>>yx[i].second>>yx[i].first;

	vector<vector<pair<ll,ll>>> dp(n+1,vector<pair<ll,ll>>(2*n+1,{-inf,-inf}));
	...
	rep(i,n){
		loop(j,ans,i*2){
			//一個も変えない場合
			if(dp[i][j]>=yx[i])dp[i+1][j]=max(dp[i+1][j],yx[i]);

			//一個変更の場合,firstを変更する場合
			if(dp[i][j].second>=yx[i].second){
				dp[i+1][j+1]=max(dp[i+1][j+1],{dp[i][j].first,yx[i].second});
			}else {
				dp[i+1][j+1]=max(dp[i+1][j+1],{dp[i][j].first-1,yx[i].second});
			}

			//1個変更の場合,secondを変更する場合
			if(dp[i][j].first>yx[i].first){
				dp[i+1][j+1]=max(dp[i+1][j+1],{yx[i].first,n-1});
			}else if(dp[i][j].first==yx[i].first){
				dp[i+1][j+1]=max(dp[i+1][j+1],{yx[i].first,dp[i][j].second});
			}
