...
ll solve(){
	...
	rep(i,n){
		loop(j,0,sum)dp[i+1][j]=dp[i][j];
		if(dp[i+1][a[i]]==-1)dp[i+1][a[i]]=i;
		loop(j,a[i]+1,sum)if(dp[i][j-a[i]]!=-1&&dp[i+1][j]==-1)dp[i+1][j]=i;
	}
	//vvdbg(dp);
	for(int k=sum/3;k>0;k--){
		//一番合計の小さい物がiになる
		if(dp[n][k]==-1)continue;
		//合計iに必要な番号を取り出し、setに入れる
		unordered_set<ll> s;
		ll tmp=k;
		while(tmp!=0){
			s.insert(dp[n][tmp]);
		...
		vvl dp2(n+1,vl(sum+1,-1));
		rep(i,n){
			loop(j,0,sum)dp2[i+1][j]=dp2[i][j];
			if(s.count(i))continue;
			if(dp2[i+1][a[i]]==-1)dp2[i+1][a[i]]=i;
			loop(j,a[i]+1,sum)if(dp2[i][j-a[i]]!=-1)dp2[i+1][j]=i;
		}
		loop(i,k,sum-2*k){
			if(dp2[n][i]!=-1){
				cout<<k<<endl;
