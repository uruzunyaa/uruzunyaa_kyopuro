...
ll solve(){	
	...
	ll ans=1;
	loop(z,1,n-1){
		//dp[i][j]=i回目の連絡後、bitパターンjの人達が知ってる確率
		vvl dp(k+1,vl(1<<n,0));
		dp[0][1<<z]=1;
		ll invM=modinv(m,mod);
		rep(i,k){
			rep(j,1<<n){
				//遷移、f番目の交友関係が選ばれた時
				rep(f,m){
					if((j&(1<<a[f]))==0&&(j&(1<<b[f]))==0){
						dp[i+1][j]+=dp[i][j]*invM;
						dp[i+1][j]%=mod;
					}else{
						dp[i+1][j|(1<<a[f])|(1<<b[f])]+=dp[i][j]*invM;
						dp[i+1][j|(1<<a[f])|(1<<b[f])]%=mod;
					}
