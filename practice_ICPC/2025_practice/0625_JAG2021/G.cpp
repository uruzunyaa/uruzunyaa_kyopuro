...
ll solve(){
	...
	vl basyo;
	rep(i,t.size()){
		if(t[i]=='1')basyo.push_back(i);
	}

	vector<vector<pair<ll,string>>> dp(200,vector<pair<ll,string>>(200,{inf,""}));
	dp[0][0]={0,""};
	...
	rep(i,t.size()){
		loop(j,0,i){
			rep(k,n){
				...
				ll cost=0;
				rep(l,cntone[k]){
					while(s[k][index]=='0')index++;
					if(l+j==basyo.size()){
						f=true;
						break;
					}
					cost+=abs(basyo[l+j]-(index+i));
					index++;
				}
				if(f)continue;
				pair<ll,string> tmp={dp[i][j].first+cost,dp[i][j].second+s[k]};
				dp[i+s[k].size()][j+cntone[k]]=min(dp[i+s[k].size()][j+cntone[k]],tmp);
			}
		}
	}

	if(dp[t.size()][basyo.size()].first!=inf)cout<<dp[t.size()][basyo.size()].second<<endl;
