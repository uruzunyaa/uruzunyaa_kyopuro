...
public:
	...
	{
		return segtree<T>(len, e, [](T a, T b)
						  { return min(a, b); });
	}
...
int solve() {
	...
	vector<lazy_segtree<ll>> sumx(GSIZE, lazy_segtree<ll>::RangeAddSumQuery(GSIZE));
	vector<lazy_segtree<ll>> sumy(GSIZE, lazy_segtree<ll>::RangeAddSumQuery(GSIZE));
	...
	rep(i, n)
	{
		...
		if (max(abs(x - s.second), abs(y - s.first)) <= r) {
			...
			sumx[max(y - r, 0LL)].update(max(x - r, 0LL), min(x + r + 1, CONSTRAINT + 1), 1);
			sumy[max(x - r, 0LL)].update(max(y - r, 0LL), min(y + r + 1, CONSTRAINT + 1), 1);
		}
	}
