...
ll solve(){
	...
	rep(i,n){
		...
		//縦横の安全地帯への移動距離
		ll dx=min(abs(x-a),abs(x-c));
		ll dy=min(abs(y-b),abs(y-d));
		if(a<=x&&x<=c)dx=0;
		if(b<=y&&y<=d)dy=0;
		ll delta=max(dx+dy-1,0LL);

		//安全地帯から次への移動距離
		ll dnx=min(abs(xx-a),abs(xx-c));
		ll dny=min(abs(yy-b),abs(yy-d));
		if(a<=xx&&xx<=c)dnx=0;
		if(b<=yy&&yy<=d)dny=0;

		//計算
		ans+=min(delta+dnx+dny,abs(x-xx)+abs(y-yy));
